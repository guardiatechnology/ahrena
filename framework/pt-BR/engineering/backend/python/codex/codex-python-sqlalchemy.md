...
Este manual define os padrões para acesso a banco de dados usando a API async do SQLAlchemy 2.0 com asyncpg, o padrão repositório para abstração de acesso a dados e o Alembic para migrações de schema. A camada de banco de dados é infraestrutura — implementa ports de domínio e NÃO DEVE vazar preocupações de ORM para a lógica de domínio.

## Context

- **Domain:** camada de acesso a banco de dados dentro de Clean Architecture.
- **Target audience:** implementadores e agentes de IA que constroem repositórios de banco de dados e migrações.
- **Update trigger:** quando os padrões SQLAlchemy evoluem ou novos padrões de banco de dados são adotados.
...
| Termo | Definição |
|-------|-----------|
| ORM Model | Classe mapeada do SQLAlchemy representando uma tabela de banco de dados |
| Repository | Abstração de acesso a dados implementando um port de domínio |
| Migration | Script de versão Alembic para mudanças de schema |
| Flush | Escrever mudanças pendentes no BD sem confirmar a transação |
