...
This Kata defines the standardized procedure for creating a new Warrior in Ahrena — from designing the identity and persona to creating the artifact in all three mandatory languages.

## When to Use

- When it is necessary to create a specialized agent with defined identity, scope, and responsibilities
- When the user explicitly requests the creation of a new Warrior
- When invoked by `cry-new-warrior`

## Inputs

| Input | Required | Description |
|-------|:--------:|-------------|
| Role | Yes | Function the Warrior performs (e.g., "Software Architect", "Code Reviewer") |
| Domain | No | Area of operation. If omitted, the agent MUST infer from the role |
| Name | No | Proper name of the Warrior. If omitted, the agent MUST suggest a thematic name |
...
- `templates/warrior-sample.md` — Official template
