...
When BDD scenarios are authored for a feature, they become the canonical statement of business intent. That statement is worth nothing if the test suite does not connect to it. Without an enforceable mapping, scenarios decay into stale documentation while tests evolve in their own direction.

This Lexis defines the mapping (scenarios are referenced by their slug from the test code itself), forbids drift in either direction (uncovered scenarios, orphan markers), and stays neutral on the test level (any level can cover a scenario, governed by `lex-test-pyramid` and `codex-test-strategy`).
