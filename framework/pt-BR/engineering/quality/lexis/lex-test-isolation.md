...
Testes que dependem de ordem, estado global mutável ou tempo real viram bombas-relógio: passam localmente, falham no CI; passam em uma máquina, falham em outra. A equipe perde horas debugando o teste em vez do código. Pior: tolerar retry como solução ensina a ignorar sinais — o próximo bug real é enterrado no ruído.

Esta Lexis existe para garantir que **cada teste seja determinístico, independente e paralelizável**, e que **testes flaky sejam tratados como bugs críticos** — não como inconveniências.
...
- **Aplica-se a:** toda suite de testes em todos os projetos.
- **Agentes vinculados:** `warrior-hera`, `warrior-apollo`, `warrior-hephaestus`.
- **Exceções:** Nenhuma. Testes lentos mas estáveis são aceitáveis no nível certo; flakiness nunca.

## Consequências de Violação

1. **Debug wasteful:** engenheiros passam horas encontrando por que o teste falha na segunda-feira e passa na terça.
2. **Perda de confiança no CI:** red builds viram ruído; merges aprovados com testes vermelhos "porque é flaky".
3. **Incidentes em prod:** race conditions não pegas em teste (porque testes mascaravam) explodem em produção.
4. **Remediação:**
   - Identificar flaky tests (report de histórico: taxa de falha).
   - Cada flaky vira ticket com prioridade P1; fix ou disable.
   - Reforçar paralelismo + ordem aleatória no CI.
...
- `lex-test-pyramid` — distribuição por nível
- `lex-frontend-testing`, `lex-python-testing`
- `warrior-hera` — conduz estratégia de testes
