...
| `<owner>/<repo>` | Não | Repositório de destino; padrão: repo atual (via git remote) | `guardiafinance/ahrena` |
...
1. **Fase 1 — Análise da Issue** (`kata-issue-analysis`): lê a issue no GitHub e busca contexto no Notion → `docs/issues/issue-{n}/01-brief.md`
2. **Fase 2 — Requisitos** (`kata-requirements-brief`): elicita ACs numerados com perspectiva PO → `docs/issues/issue-{n}/02-requirements.md`
3. **Fase 3 — Arquitetura** (`kata-architecture-brief`): mapeia componentes, delega design de API/eventos a Daedalus/Kronos se aplicável, invoca `kata-adr-write` para decisões relevantes → `docs/issues/issue-{n}/03-architecture.md` + ADRs em `docs/adr/`
...
6. **Fase 5 — Revisão de Segurança** (`kata-security-review`): OWASP + CVE scan → `docs/issues/issue-{n}/05-security-review.md`
7. **Fase 6 — Gate 2 Qualidade** (`kata-quality-gate`): 6 checks (rastreabilidade AC↔teste, scope creep, best practices, testes, cobertura, tipos) → `docs/issues/issue-{n}/06-quality-report.md`; `no-go` retorna à Fase 4
...
1. **Fase 1:** kata-issue-analysis — leia a issue via GitHub MCP e busque contexto via Notion MCP; produza o brief em docs/issues/issue-{n}/01-brief.md.
...
- Athena lê issue #42, produz `docs/issues/issue-42/01-brief.md`
...
- Athena cria PR e informa URL: `https://github.com/guardiafinance/ahrena/pull/123`
...
- **Documentação em `docs/`:** todos os artefatos públicos do fluxo ficam em `docs/issues/issue-{n}/` e `docs/adr/`
