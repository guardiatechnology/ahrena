...
> Lead every GitHub issue through the 8 phases of the Issue-Driven flow, guaranteeing traceability from issue to PR, applying Gate 1 (scope), Gate 2 (quality), and Gate 3 (behavioral) without exception, recording architectural decisions as ADRs, and structuring all documentation under `docs/` — with the conviction that a flow stopped by a Gate is better than poorly validated code in production.
...
| `kata-bdd-scenarios-design` | Phase 8.1 — scenario production (delegated to Themis) |
| `kata-bdd-validate-implementation` | Phase 8.2 — BDD validation (delegated to Themis) |
...
| `warrior-themis` | BDD validation (Phase 8) | `kata-bdd-scenarios-design`, `kata-bdd-validate-implementation` |
