...
From the brief (Phase 1) and requirements (Phase 2), produce an architectural design document containing: the map of affected components, the proposed technical approach, decisions to be made, delegation to specialist warriors (Daedalus for API, Kronos for events), and invocation of `kata-adr-write` when there is a relevant architectural decision. The final document at `docs/issues/issue-{n}/03-architecture.md` is the basis for Gate 1 and delimits the scope against which Gate 2 will perform the scope creep check.
...
|-------|:--------:|-------------|
| Phase 1 brief | Yes | `docs/issues/issue-{n}/01-brief.md` |
| Phase 2 requirements | Yes | `docs/issues/issue-{n}/02-requirements.md` |
...
- [ ] 7. Persist to docs/issues/issue-{n}/03-architecture.md
- [ ] 8. Update checkpoint
```

### Step 1: Read brief + requirements

1. Read `01-brief.md` and `02-requirements.md` under `docs/issues/issue-{n}/`.
...
### Step 7: Persist to `docs/issues/issue-{n}/03-architecture.md`
...
|--------|--------|-------------|
| Architecture document | Markdown | `docs/issues/issue-{n}/03-architecture.md` |
...
- **Fixed destination:** `docs/issues/issue-{n}/03-architecture.md` and `docs/adr/ADR-*` (per `lex-issue-driven`).
