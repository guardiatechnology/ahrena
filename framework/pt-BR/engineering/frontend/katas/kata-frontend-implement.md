...
Dada uma feature frontend (componente, página, fluxo de interação), produzir implementação completa em TypeScript com testes comportamentais, acessibilidade, tipos estritos e aderência aos padrões arquiteturais do projeto. Inclui componentes de UI, hooks/composables, integração com API, gerenciamento de estado e testes.

## Quando Usar

- Quando uma feature frontend foi descrita (brief + requisitos) e precisa ser implementada
- Invocada por `warrior-hephaestus` diretamente ou via delegação de `warrior-athena` na Fase 4 do fluxo Issue-Driven

## Inputs

| Input | Obrigatório | Descrição |
|-------|:-----------:|-----------|
| Descrição da feature | Sim | Requisitos + critérios de aceitação (ACs numerados se vindo do fluxo Issue-Driven) |
| Contrato de API | Não | OAS ou tipos de request/response (ideal: produzido por `warrior-daedalus`) |
| Design ou wireframe | Não | Figma, imagens ou descrição textual de UI |
| Stack detectado | Sim | Framework (React/Next/Vue/Angular), linguagem (TS), ferramentas (testing, styling) |
...
- `codex-frontend-architecture` — padrões de estrutura e camadas
- `warrior-hephaestus` — agente que executa este kata
- `kata-api-design-oas` — contrato de API produzido por Daedalus
