...
Quick command to create a new Cry in Ahrena. Invokes `kata-create-cry`, which consults `codex-cries` and the official template to produce a complete recurring command in all three mandatory languages. This is the Cry that creates Cries — the shortcut to the self-replication mechanism.
...
2. Consults `codex-cries` for quality criteria
3. Reads `templates/cry-sample.md` as the structural base
4. Checks whether the associated Kata exists
5. Executes `kata-create-cry` with the provided parameters
6. Creates the Cry in the default language and translates to the remaining languages
7. Reports the created files
...
| **Complexity** | Low (action + kata) | High (command design, prompt, validation) |
| **Configures agent?** | No | Yes (defines behavior) |
| **Example** | `/cry-new-cry "generate changelog"` | 6-step workflow with checklist |

## References

- `kata-create-cry` — Procedure executed by this Cry
- `codex-cries` — Quality criteria consulted
- `templates/cry-sample.md` — Base template
