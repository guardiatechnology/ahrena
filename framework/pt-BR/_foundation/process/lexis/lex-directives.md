...
| `pr_cost_tracking.idle_gap_minutes` | Gap (em minutos) que separa janelas ativas dentro de uma sessão Claude Code para o cálculo de tempo ativo. Default `10`. Valor menor torna a contagem mais estrita; maior agrega pausas longas. Subchaves `currency`, `include_cache_breakdown`, `window_override_days`, `mask_absolute_cost` permanecem declaradas em `.directives.sample` como reservadas para iterações futuras. |
