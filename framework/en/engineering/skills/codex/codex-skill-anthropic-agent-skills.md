...
Agent Skills is an open standard promoted by Anthropic to package modular capabilities that extend AI agents. Each Skill is a directory with a `SKILL.md` file (YAML frontmatter + Markdown) that the agent loads progressively: metadata always, body when activated, resources on demand.

This Codex is the conceptual and field reference for the spec. It does not cover the Ahrena convention for bundling MCP tools and React widgets — that lives in `codex-skill-tools-and-widgets`. It does not cover the source project layout in the repository — that lives in `codex-skill-project-architecture`.

Canonical source of the spec: [agentskills.io/specification](https://agentskills.io/specification). Anthropic documentation: [platform.claude.com/docs/en/agents-and-tools/agent-skills/overview](https://platform.claude.com/docs/en/agents-and-tools/agent-skills/overview).

## Context

- **Domain:** external skill format (final delivery consumed by agents outside Ahrena)
- **Audience:** skill authors, `kata-init-skill`, `kata-build-skill`, `kata-package-skill` (future PRs)
- **Update:** when the official spec evolves; review `metadata.spec_version` of the produced skill
...
Free-form map. Ahrena conventions (not from the spec) consumed by `kata-build-skill`:
...
Checks valid frontmatter, naming, and minimum structure. `kata-build-skill` (PR 2) will integrate this validation.
...
Skills produced in Ahrena are auditable through the commit trail (refs snapshotted with hash, deterministic manifest in `lex-skill-export-determinism` in PR 3).

## Restrictions

- The spec **does not define** layout for MCP tools nor UI widgets. The Ahrena convention (`codex-skill-tools-and-widgets`, in PR 2) creates additional `tools/` and `widgets/` directories — agents that only know the spec ignore these directories. Documenting the convention as an "Ahrena extension" is mandatory in the generated SKILL.md.
...
| Term | Definition |
|------|------------|
| Agent Skills | Anthropic's open standard for filesystem-based skills |
| SKILL.md | Root file with frontmatter + body |
| Progressive disclosure | 3-level loading (metadata, instructions, resources) |
| Pre-built Skill | Anthropic skill available without upload (PowerPoint, Excel, Word, PDF) |
| Custom Skill | Skill created by a third party, distributed via filesystem or upload |

## References

- [Canonical spec — agentskills.io/specification](https://agentskills.io/specification)
- [Anthropic documentation — Agent Skills overview](https://platform.claude.com/docs/en/agents-and-tools/agent-skills/overview)
- [Open repo — anthropics/skills](https://github.com/anthropics/skills)
...
- `codex-skill-project-architecture` — Ahrena source project layout (`skills/{slug}/`)
- `codex-skill-tools-and-widgets` — Ahrena convention for `tools/` (MCP) and `widgets/` (React) [PR 2]
- `lex-skill-project-structure` — law of the source project layout
