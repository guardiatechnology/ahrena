...
Behavior-Driven Development at Guardia is an optional, standalone discipline used when the gap between numbered acceptance criteria and the underlying business intent is wide enough to deserve a separate, business-language layer. When BDD is adopted for a feature, the source of truth for that intent must be unambiguous — otherwise scenarios drift toward describing the contract or the code, and stop describing the business.

This Lexis fixes the source (issue + Notion), the language (domain-only), and the persistence (the issue body itself) so that every later step (test mapping, validation, reviews) reads from the same canonical surface.
