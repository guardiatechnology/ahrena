...
5. **Phase 3 — Consolidation:**
   ...
   - Lists open comments from other reviewers (`gemini-code-assist`, `coderabbitai`, `Copilot`, `qodo-merge-pro`, humans) via `gh api repos/{owner}/{repo}/pulls/{N}/comments` filtering by `user.login` ≠ `ahrena-warrior-argos[bot]`; aggregates into a `## 🧭 Pending threads from other reviewers` subsection in the consolidated body when open threads exist (omits the subsection when the list is empty). This is an aid to the multi-reviewer sweep required by Rule 8 of `lex-pr-quality`, not a substitute — the agent applying the fixes MUST still run its own sweep
