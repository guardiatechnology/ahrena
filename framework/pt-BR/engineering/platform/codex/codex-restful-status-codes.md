...
Códigos de status permitidos e regras de uso para endpoints HTTP da plataforma Guardia. Códigos utilizados em cada endpoint DEVEM constar no contrato OAS. Padrão mínimo para qualquer API RESTful da Guardia.

## Contexto

- **Domínio:** códigos de status HTTP em respostas de API.
- **Público-alvo:** implementadores e consumidores de APIs.
- **Atualização:** quando a especificação de status codes no Hub for alterada.
