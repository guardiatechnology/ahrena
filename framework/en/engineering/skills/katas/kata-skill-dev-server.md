...
Enable rapid iteration on a skill project by bringing up, in a single invocation, the three servers that cover widgets, scripts, and tools. The kata respects opt-outs from `skill.config.json` — if the project has no `widgets/`, the widgets server is not started; the same applies to scripts and tools.

## When to Use

- When the user invokes `/cry-skill-dev <slug>`
- Before running `kata-build-skill`, to manually validate widgets, scripts, and tools
- When adjusting bindings in `widgets/manifest.json` and validating `called_via`

## Inputs

| Input | Required | Description |
|-------|:--------:|-------------|
| `slug` | Yes | Project name at `{paths.skills_root}/{slug}/` |
| `widgets_port` | No | Override of the widgets port (default: `skill.config.json` → `dev_server.widgets_port` → `5173`) |
| `scripts_port` | No | Override of the script runner port (default: `5174`) |
| `tools_stub_port` | No | Override of the tool stub port (default: `5175`) |
| `only` | No | Subset to bring up (`widgets`, `scripts`, `tools`); default: all available |
...
- `codex-skill-build-pipeline` — tooling defaults, ports, build phases (preconditions of the dev server and what it simulates)
- `codex-skill-tools-and-widgets` — schemas of `mcp.config.json` and `manifest.json` validated by the preconditions
- `codex-skill-project-architecture` — dev → build → dist flow
- `codex-frontend-architecture` — Vite and dev server conventions applicable to widgets
- `lex-skill-project-structure` — layout law
- `lex-terminal-type` — terminal and command syntax
- `cry-skill-dev` — user shortcut
- `kata-build-skill` — natural next step after validation in dev
