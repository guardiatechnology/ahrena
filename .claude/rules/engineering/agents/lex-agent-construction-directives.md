...
(2) `direct-entry` — Mêtis invoked to design an agent directly into
    ...
    DoOC items (a)-(e) may be filled as `N/A — direct-entry` in
    `dooc/{agent}.md`, always referencing the ADR/PDR; items (f)-(i)
    remain mandatory.
...
(3) `user-override` — the user (CEO or designated Brand owner) promotes
    ...
    or PDR declaring:
      ...
      (iii) retroactive compensation in a declared window
            (suggested: 30 days).
    Overridden items appear as `N/A — user-override` in the snapshot.
...
or PDR, NOT a bypass of the gate. Without a valid ADR/PDR,
`direct-entry` and `user-override` are non-compliant.
</HARD-GATE>
```
