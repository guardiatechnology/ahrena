...
Build or update a repository's knowledge graph with Graphify and query it to produce the affected-components table in the format `kata-architecture-brief` consumes. The procedure delivers direct **and reverse** dependencies, which is the gap left by reading the repository ad hoc.

The graph is advisory input. This Kata never blocks a flow: when Graphify is absent, disabled, or the graph is stale, it records the unavailability and returns control to the calling agent.

## When to Use

- In step 2 of `kata-architecture-brief`, when mapping the components affected by a set of acceptance criteria
- In the scope-creep check of `kata-quality-gate`, when comparing the PR diff against the declared scope boundary
- When assessing the impact radius of a change to a public contract, before altering it
- When investigating an unfamiliar codebase and needing to identify architectural hubs

Do not use it when the answer requires a single read of an already known file. The cost of building the graph does not justify it.

## Inputs

| Input | Required | Description |
|-------|:--------:|-------------|
| Repository path | Yes | Root of the repository to index |
| Nodes of interest | No | Labels or IDs to query (`affected`, `explain`, `path`) |
| Acceptance criteria | No | Required when the output feeds the `kata-architecture-brief` components table |
| Extraction mode | No | `code-only` (default) or semantic. See `codex-graphify` |
