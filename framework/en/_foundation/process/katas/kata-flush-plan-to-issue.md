...
Persist the content of `.plans/{N}.md` (AI working memory) to the GitHub Issue body (canonical), filtering local blocks marked `<!-- not-flushed -->` ... `<!-- /not-flushed -->`. Idempotent operation. Triggered at the 3 canonical triggers of `lex-agent-planning`: each `status:` label transition, each completed Step, and end of session.

## When to Use

- `status:` label transition on the Issue/PR (`todo → development`, `development → to review`, etc.).
- Plan step marked complete (`[ ]` → `[x]`).
- End of Claude Code session (heartbeat finishes or the Athena/Argos/Janus agent exits).
- Handoff between agents (deliver before the next agent enters).
- Explicit user request ("flush plan", "update the Issue").
...
| `source_path` | No | Path of the local cache file. Default: `<paths.plans>/{N}.md` |
| `force` | No | `true` forces write even if an unknown remote edit occurred. Default: `false` (alert + offer manual merge) |
...
- `lex-agent-planning` — 3-layer model and flush cadence (3 canonical triggers)
...
- `kata-load-plan-from-issue` — inverse operation (Issue → cache)
- `kata-pr-prepare` — invokes `kata-flush-plan-to-issue` before opening the PR
- `warrior-athena`, `warrior-argos`, `warrior-janus` — agents that trigger flush on transitions
