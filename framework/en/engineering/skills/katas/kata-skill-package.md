...
Produce, from a validated source project, the final `.skill/` package consumed by external agents in the Anthropic Agent Skills format. The packager is deterministic: given the same source and the same framework commit, the output is byte-identical, with `.skill-manifest.json` lexicographically sorted, every SHA-256 hash verified, and zero orphan files.

## When to Use

- After `kata-skill-validate` returns `ok` or `ok-with-warnings`
- When `warrior-claudionor` needs to deliver the final package under `{paths.skills_dist}/`
- During Phase 2 (build → dist) of a skill release
...
| `repo_root` | No | Repository root (default: current working directory) |
| `skills_root` | No | Override of `paths.skills_root` (default: read from `.ahrena/.directives`) |
| `skills_build` | No | Override of `paths.skills_build` |
| `skills_dist` | No | Override of `paths.skills_dist` |
| `dry_run` | No | `true` validates the source but writes nothing under build/dist |
| `format` | No | `text` (default, human) or `json` |
...
- `scripts/skills/package.py` — deterministic implementation invoked
- `lex-skill-package-structure` — law being verified (5 criteria + HARD-GATE)
- `lex-skill-project-structure` — precondition (source must be valid)
- `lex-semantic-version` — versioning of `skill.version`
- `kata-skill-validate` — prerequisite (same runner)
- `warrior-claudionor` — orchestrator that invokes this kata
- `cry-skill` — shortcut `--mode package`
