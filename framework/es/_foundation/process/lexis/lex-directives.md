...
| `pr_cost_tracking.known_ai_reviewers` | Lista de logins GitHub adicionales reconocidos como revisores AI en la subsección Review. Built-ins (gemini-code-assist[bot], claude[bot], coderabbitai[bot], qodo-merge-pro[bot], ahrena-warrior-argos[bot]) siempre se reconocen; los logins aquí extienden el conjunto. Las subclaves `currency`, `include_cache_breakdown`, `window_override_days`, `mask_absolute_cost` permanecen declaradas en `.directives.sample` como reservadas para iteraciones futuras. |
| `pr_cost_tracking.known_ai_authors` | Lista de logins GitHub adicionales reconocidos como **autores** AI en la subsección Development (simétrico a `known_ai_reviewers`). Built-ins (`ahrena-bot[bot]`, `claude[bot]`, `copilot[bot]`) siempre se reconocen; los logins aquí extienden el conjunto. Conduce la clasificación de autor-bot descrita en `kata-pr-cost-stamp` § "Identidad del autor": cuando el login de autor del PR pertenece a la unión de built-ins + lista del proyecto, el bloque de costo emite `Bot-authored: yes (<login>)` y reformula la subsección Total. |
| `rtk.enabled` | Cuando es `true` (default), `scripts/install.py` instala el hook `PreToolUse` de RTK (Rust Token Killer) en el `.claude/settings.json` del proyecto destino con matcher `"Bash"` y el comando con fallback estricto `if command -v rtk >/dev/null 2>&1; then rtk hook claude; fi` (no-op cuando el binario está ausente del PATH). Cada install/update reconcilia el hook de forma idempotente. Cuando es `false`, install/update NO tocan ningún artefacto relacionado con RTK (no agrega, no elimina). |
| `rtk.auto_install_binary` | Cuando es `true` (default), `scripts/install.py` detecta si el binario `rtk` está en el PATH y lo instala cuando está ausente. La ruta de instalación es OS-aware: `brew install rtk` (macOS, cuando Homebrew está disponible), `curl install.sh \| sh` (Linux y fallback en macOS), WSL/cargo (Windows). Las fallas de instalación no son fatales — el hook mantiene la forma de fallback estricta, así que un binario ausente nunca rompe Claude Code. Establece `false` para saltar el intento de instalación del binario manteniendo solo el wire del hook. |
| `graphify.enabled` | Cuando es `true`, `scripts/install.py` habilita Graphify (grafo de conocimiento de código) en el proyecto destino: detecta o instala el binario `graphify` e informa la disponibilidad de `graphify-mcp`. A diferencia de RTK, el default cuando la sección se omite es `false` — Graphify es una feature opcional y trae un árbol de dependencias grande (cerca de 36 gramáticas tree-sitter), así que una instalación que no lo pidió permanece limpia. No se registra ningún hook: el grafo se consulta bajo demanda mediante `kata-codebase-graph`, nunca en cada llamada de Bash. Ver `codex-graphify`. |
| `graphify.auto_install_binary` | Cuando es `true` (default), `scripts/install.py` detecta si el binario `graphify` está en el PATH y lo instala cuando está ausente, mediante `uv tool install graphifyy` con fallback a `pipx install graphifyy`. Ambos aíslan la herramienta en su propio entorno y no contaminan el venv del proyecto. Las fallas de instalación no son fatales — el grafo es insumo consultivo, así que `kata-codebase-graph` degrada de forma limpia y nada se rompe. Establecer `false` para saltar el intento de instalación manteniendo el contrato de la directriz. |
| `graphify.mode` | `code-only` (default) ejecuta solo extracción AST local: determinista, sin clave de API y sin red. `semantic` habilita la etapa opcional con modelo de lenguaje sobre documentos, PDF e imágenes. Ningún gate de CI debe depender del modo `semantic`. Ver el modelo de costo medido en `codex-graphify`. |
...
| `notifications.channels.release_notify` | Canal lógico para anuncio de release concluida (Janus). |
| `notifications.channels.plans_status` | Canal lógico para el digest periódico de planes activos (Eunomia). |
...
| `warriors_default_author.enabled` | Llave maestra para la identidad predeterminada de los warriors en commits/PRs. Cuando `true`, los warriors listados en `warriors_default_author.apply_to` invocan `scripts/ahrena-auth.sh` antes de `git commit` / `gh pr create`, de modo que los commits y PRs se atribuyen a la identidad `[bot]` del GitHub App predeterminado (firmada en el servidor mediante el token de instalación del App) y el ser humano que condujo el trabajo se registra como `Co-authored-by:`. Default `false` — el comportamiento actual de autoría humana se preserva bit a bit hasta que el proyecto opte explícitamente. Ver `codex-git-workflow` ("Identidad de autor"). |
| `warriors_default_author.identity` | Slug del GitHub App usado para derivar la identidad predeterminada de los warriors (default `ahrena-bot`). Sobreescriba sólo cuando un fork o clon utilice otro slug. |
| `warriors_default_author.commit_mode` | `api` (firmado en el servidor vía token de instalación del App) — único modo disponible hoy. El valor `local` está reservado para una iteración futura. |
| `warriors_default_author.commit_co_author` | `human` (inyecta `Co-authored-by: <nombre humano> <email humano>` para que la persona que condujo el trabajo permanezca rastreable) o `none`. |
| `warriors_default_author.apply_to` | Lista de nombres de warriors que respetan el override. Los warriors omitidos de la lista mantienen el autor humano aunque la llave maestra esté activada (opt-out por warrior). Los warriors con su propio GitHub App (ej.: Argos consume `AHRENA_WARRIOR_ARGOS_GH_*`) no dependen de esta lista. |
