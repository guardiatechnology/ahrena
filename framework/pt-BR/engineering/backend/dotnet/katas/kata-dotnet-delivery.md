...
Executar trabalho .NET com descoberta do contrato do repositório, modelagem proporcional, testes por risco e validação operacional completa.

## Quando Usar

- Para implementar feature ou correção em C#/.NET
- Para revisar ou refatorar código .NET
- Para diagnosticar falha de build, runtime, dados ou produção

## Inputs

| Input | Obrigatório | Descrição |
|---|:---:|---|
| Modo | Sim | `implement`, `review`, `refactor` ou `debug` |
| Objetivo | Sim | Comportamento, diff ou falha em escopo |
| Evidência | Não | Issue, logs, stack trace, contrato, métricas ou reprodução |
