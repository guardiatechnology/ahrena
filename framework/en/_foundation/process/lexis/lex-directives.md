...
| `rtk.auto_install_binary` | Reserved placeholder (default `false`). The actual auto-install logic requires a follow-up ADR — RTK has no native auto-updater and the install channel varies per environment (`cargo install`, `brew install`, `curl \| sh`, npm). When `false` (default), install/update logs a warning if `command -v rtk` returns false but does not install. |
