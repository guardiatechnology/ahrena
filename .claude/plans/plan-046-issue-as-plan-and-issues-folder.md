...
2. ~~**`.issues/{N}/` numbering sem prefix `issue-`?**~~ **Resolvido em 2026-05-11:** sim, `.issues/3/` (não `.issues/issue-3/`). O `.issues/` já implica que o conteúdo é Issue-related; prefixar `issue-` é ruído. Aplicar `git mv docs/issues/issue-{N} .issues/{N}` em repos consumidores no Step 5.
3. ~~**Cache local sync cadence (`kata-flush-plan-to-issue`):**~~ **Resolvido em 2026-05-11:** sincronização em 3 gatilhos canônicos — (a) cada transição de `status:` (todo→development, etc.), (b) cada Step do plano concluído (passa de `[ ]` para `[x]`), (c) fim de sessão (heartbeat conclui ou agente Athena/Argos sai). Toggles intermediários e edições de scratch são livres (não disparam flush). Documentar os 3 gatilhos em `codex-agent-planning` §9.
4. ~~**Schema do `.plans/{N}.md`:**~~ **Resolvido em 2026-05-11:** superset do body da Issue. `.plans/{N}.md` carrega o body completo (espelhado) + seções locais marcadas com `<!-- not-flushed -->` ... `<!-- /not-flushed -->` que `kata-flush-plan-to-issue` filtra antes de gravar. Seções locais típicas: `## Working notes`, `## Next actions`, `## Scratch` — para a IA registrar decisões em rascunho, observações de debugging, próximos passos voláteis sem poluir o Issue body canônico.
5. ~~**`gh issue edit` sem MCP**:~~ **Resolvido em 2026-05-11:** per `lex-mcp` regra 1, usar **MCP `update_issue` (GitHub MCP)** quando o servidor estiver listado em `mcp.servers` e ativo; `gh issue edit --body-file` como **fallback CLI documentado** quando MCP indisponível (per `lex-mcp` regra 4: retry único, depois oferecer escolha CLI/pausar/abortar). Tanto `kata-flush-plan-to-issue` quanto `kata-load-plan-from-issue` codificam essa preferência.
...
**Todas as 7 Open Questions resolvidas em 2026-05-11.** Plan-046 está pronto para execução em sessão futura — basta abrir Issue, branch, worktree (Step 1) e seguir.
