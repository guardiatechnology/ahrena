...
O Ahrena organiza todo conhecimento em cinco Pilares — Lexis, Codex, Katas, Warriors e Cries. Para que o framework seja auto-contido e auto-validável, é necessário que as definições e as regras de cada Pilar sejam explícitas e inquebráveis. Sem uma Lei que defina o que cada Pilar é e o que cada um pode ou não fazer, não há critério objetivo para validar artefatos nem para garantir consistência entre criação, invocação e evolução do framework.

Esta Lexis existe para estabelecer as **definições e regras canônicas dos cinco Pilares**, de forma que todo artefato possa ser validado contra ela e toda invocação respeite a hierarquia e as relações entre Pilares.
...
- **Aplica-se a:** todo artefato criado ou mantido no framework Ahrena e toda cadeia de invocação (Cry → Warrior/Kata → Kata → Lex/Codex).
- **Agentes vinculados:** todos os Warriors e agentes genéricos que criam, validam ou invocam artefatos do framework.
- **Exceções:** Nenhuma. Lexis não admitem exceções.

## Consequências de Violação

1. **Artefato inválido:** artefato que não satisfaz a definição do seu Pilar não deve ser aceito no framework; deve ser corrigido ou reclassificado.
2. **Invocação inválida:** Cry que invoque Lexis ou acesse Codex diretamente viola a Lei; o design do Cry deve ser corrigido para delegar a Kata ou Warrior.
3. **Remediação:** o agente ou revisor deve consultar `lex-pilars` e `codex-pilars` para validar o artefato e a cadeia de invocação e corrigir desvios.
...
- `codex-pilars` — Referência central dos Pilares e checklists de validação por Pilar
- `codex-pilars` — Referência central do sistema de Pilares e relações
- `.ahrena/.directives` — Prefixos e paths (naming.prefixes, paths.samples)
- `lex-template-usage` — Uso obrigatório do template por Pilar
