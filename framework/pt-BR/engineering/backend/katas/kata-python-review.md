...
Esta Kata define o procedimento para revisar mudanças de código Python: verificar corretude, segurança de tipos, cobertura de testes, segurança, tratamento de erros e aderência aos Lexis do projeto. O objetivo é detectar bugs, aplicar padrões e compartilhar conhecimento — não fazer de guardião nem criticar estilo (Ruff cuida disso).

## When to Use

- Quando se revisa um pull request ou mudança de código em um serviço Python backend
- Quando invocado por `cry-python-review` ou pelo Warrior Apollo
- Quando o usuário solicita uma revisão de código ou avaliação de qualidade

## Inputs

| Input | Obrigatório | Descrição |
|-------|:-----------:|-----------|
| Código a revisar | Sim | Diff, caminhos de arquivos ou referência de PR contendo as mudanças |
| Contexto | Não | Descrição do feature, issue relacionada ou documento de design |
