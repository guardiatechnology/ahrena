...
| `--force` | Não | Sobrescreve PoV existente no mesmo `--context` | (flag) |

## O que o Comando Faz

1. Resolve `--context` e prepara `docs/{context}/agents-pov/`
...
4. Reporta o tree final do `docs/{context}/agents-pov/` + paths dos artefatos de implementação
...
docs/{{context}}/agents-pov/. Aplique as 6 Diretrizes de Construção
...
Fase 1/8 — kata-pov-scope-define
   ✅ overview.md criado (caso de uso primário: pareamento extrato↔lançamento;
...
- `--context` deve ser kebab-case e único; se já existe `docs/{context}/agents-pov/`, exige `--force` para sobrescrever.
