...
Este Codex é a referência para decisões arquiteturais em frontend: como organizar componentes, onde fica a lógica de negócio, como gerenciar estado, como estruturar roteamento, como abstrair a camada de dados. Consultado por `warrior-hephaestus` e por agentes que implementam features frontend.

Referência principal assume **React + TypeScript + Next.js**, mas os princípios são transferíveis para Vue (Composition API + Pinia), Angular (standalone components + Signals) e Svelte (SvelteKit + stores).

## Contexto

- **Domínio:** arquitetura de aplicações frontend SPA ou SSR
- **Público-alvo:** `warrior-hephaestus`, agentes que implementam UI
- **Atualização:** quando o framework principal do projeto muda, quando novos padrões emergem (Server Components, Signals, etc.)
...
- `lex-frontend-typing` — tipagem estrita
- `lex-frontend-testing` — estratégia de testes
- `lex-frontend-accessibility` — WCAG obrigatório
