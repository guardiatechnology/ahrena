...
Shortcut to create a new skill project under `{paths.skills_root}/{slug}/` (default `skills/{slug}/`) from the official template. Invokes `kata-init-skill`, which validates the slug against the Anthropic Agent Skills spec, copies `framework/templates/skill-project-sample/`, replaces placeholders, and ensures `.gitignore` contains `.build/`.
...
- cry-skill-dev / cry-skill-build land in PR 2
```
...
- `kata-init-skill` — invoked procedure
- `codex-skill-anthropic-agent-skills` — slug, description, frontmatter rules
