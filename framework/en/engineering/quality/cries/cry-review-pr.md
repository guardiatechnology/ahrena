...
This command invokes `warrior-argos` to run a structured multi-axis review on an open Pull Request: collects context from GitHub (PR diff, view, checks, linked Issue) and Notion (PRD, Capability Spec) when available; creates an isolated worktree; runs the applicable review katas (Python, frontend, AWS, OpenAPI, CloudEvents, security); executes tests locally; detects breaking changes in public contracts; and consolidates findings into a single idempotent review-comment posted via `gh pr review --request-changes` (or `--comment` when no findings). Never `--approve` — approval remains a human decision.
...
- Phase 0: Collect PR, linked Issue, PRD/Capability Spec on Notion (when available), local docs/issues/issue-{N}/, and the referenced plan
...
Phase 0 — Collection: Issue #138 ✅ | PRD on Notion ✅ | docs/issues/issue-138/ ✅
...
| Aspect | Cry | Kata |
|--------|-----|------|
| **Nature** | Quick invocation with PR number | Full multi-step orchestration |
| **Complexity** | Low (one command) | High (Argos runs 5 phases and 6 axes) |
| **Configures agent?** | Yes (assumes Argos Warrior role) | N/A (Argos is a Warrior, not a Kata) |
| **Example** | `/cry-review-pr 142` | Argos invokes `kata-python-review`, `kata-events-review`, etc. |
...
- `warrior-argos` — Warrior invoked by this Cry
- `lex-issue-first`, `lex-pr-quality`, `lex-issue-driven` — Laws applied during the review
- `lex-mcp` — MCP behavior when GitHub or Notion is unavailable
- `cry-implement-issue` — Pre-PR counterpart (`warrior-athena` Gate 2 on the author side)
