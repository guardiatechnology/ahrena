...
Buscar e ler conteúdo do Notion (páginas, databases, blocos) via servidor MCP, trazendo as informações para o contexto da sessão para referência, análise ou processamento. Esta kata é estritamente **somente leitura** — nenhuma página ou bloco é criado ou modificado.

## Quando Usar

- Quando o usuário precisa consultar documentação, notas ou decisões registradas no Notion
- Quando é necessário obter entradas de um database do Notion (ex.: backlog, ADRs, tarefas)
- Quando o conteúdo de uma página Notion deve ser analisado ou referenciado durante a sessão

## Inputs

| Input | Obrigatório | Descrição |
|-------|:-----------:|-----------|
| Consulta ou ID | Sim | Termo de busca textual, ID de página ou URL da página/database no Notion |
| Modo | Não | `search` (busca por texto), `page` (página específica por ID), `database` (entradas de database); padrão: `search` |
| Filtro de database | Não | Filtro de propriedades para `query_database` (ex.: status, data, tag) |
| Profundidade | Não | `summary` (título + resumo) ou `full` (conteúdo completo dos blocos); padrão: `summary` |
