...
This Kata defines the procedure to **review an existing API contract** (OpenAPI 3.x specification or Markdown document) against Guardia's RESTful Lexis and Codex, identifying compliance violations, gaps, and improvements, and producing a structured review report with severity-classified findings.

## When to Use

- When an existing OAS file or API Markdown document needs validation against Guardia rules before or after implementation
- When a PR includes changes to an API contract that must pass Gate 2 (kata-quality-gate)
- When invoked by Warrior Daedalus as part of a design-review cycle
- When `cry-api-review` is triggered by the user

## Inputs

| Input | Required | Description |
|-------|:--------:|-------------|
| API contract path | Yes | Path to the OAS file (YAML/JSON) or Markdown API document to be reviewed |
| Review scope | No | Specific endpoints or rules to focus on. If omitted, reviews the full contract |
...
- `kata-events-review` — symmetric pair for CloudEvents contracts
- `warrior-argos` — orchestrator that invokes this kata during a multi-axis Pull Request review
