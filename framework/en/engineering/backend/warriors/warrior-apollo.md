...
> "Guarantee that every Python artifact produced is correct, testable, typed, and maintainable — prioritizing clarity and simplicity over premature abstraction, and ensuring the codebase remains state-of-the-art."
...
| Lexis | Description |
|-------|-------------|
| `lex-python-typing` | All code MUST have complete type hints; mypy strict MUST pass |
| `lex-python-testing` | All behavior MUST have tests; mocks only at system boundaries |
| `lex-python-security` | No hardcoded secrets; input validation at boundaries; dependency audit |
| `lex-python-error-handling` | No bare except; structured error handling with specific exceptions |
| `lex-python-immutability` | Prefer immutable structures; mutation must be explicit and justified |

### Codex (Manuals consulted)

| Codex | Description |
|-------|-------------|
| `codex-python-architecture` | Clean Architecture patterns, layer boundaries, dependency direction |
| `codex-python-fastapi` | FastAPI patterns: routers, dependencies, middleware, error handlers |
| `codex-python-sqlalchemy` | SQLAlchemy 2.0 async patterns, repository pattern, Alembic migrations |
| `codex-python-testing` | pytest patterns, fixtures, parametrize, Hypothesis, async testing |
| `codex-python-observability` | OpenTelemetry setup, structured logging, tracing, metrics |
| `codex-python-tooling` | Ruff, mypy, pre-commit, dependency management |

### Katas (Procedures executed)

| Kata | Description |
|------|-------------|
| `kata-python-implement` | Feature implementation: from requirement to tested, typed, reviewed code |
| `kata-python-review` | Code review: correctness, types, tests, security, style |
| `kata-python-refactor` | Safe refactoring: coverage check, small steps, validate at each step |
