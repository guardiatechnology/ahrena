...
Acessibilidade não é feature opcional — é requisito legal (LGPD art. 18, Lei Brasileira de Inclusão, ADA, EAA europeia) e ético. Uma UI inacessível exclui usuários com deficiência visual, motora, cognitiva ou auditiva, e empobrece a experiência de todos (teclado, dispositivos assistivos, ambientes ruidosos). Para agentes IA que geram código frontend, ignorar acessibilidade é construir dívida legal e humana.

Esta Lexis existe para garantir que **toda UI produzida atenda no mínimo WCAG 2.1 nível AA**, que **elementos interativos sejam navegáveis por teclado**, que **conteúdo seja anunciado corretamente por leitores de tela** e que **contraste de cores seja adequado**.
...
- **Aplica-se a:** toda UI renderizada, incluindo componentes de bibliotecas terceiras (se uma biblioteca não é acessível, substituir ou contribuir com fix)
- **Agentes vinculados:** `warrior-hephaestus` e qualquer outro agente que gere frontend
- **Exceções:** Nenhuma. Lexis não admitem exceções.

## Consequências de Violação

1. **Exclusão de usuários:** pessoas com deficiência não conseguem usar a aplicação — problema ético e de alcance
2. **Risco legal:** LGPD, Lei Brasileira de Inclusão (LBI), EAA podem resultar em ações regulatórias ou judiciais
3. **SEO prejudicado:** sites inacessíveis têm pior ranqueamento (Google usa sinais de acessibilidade)
4. **Remediação:** rodar axe/Lighthouse; corrigir violações críticas imediatamente; adicionar testes de acessibilidade no CI
...
- `lex-frontend-testing` — queries acessíveis em testes
