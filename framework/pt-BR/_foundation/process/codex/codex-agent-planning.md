...
Este Codex é o manual canônico de planejamento de tarefas por agentes no modelo hierárquico **Issue → Plan → PR**. Complementa `lex-agent-planning` (a Lei) com templates, exemplos de preenchimento, walkthroughs Top-down e Plan-first, cadência de load/flush, owners de cada transição e diretrizes para casos-limite. Todo agente que cria ou mantém Plans DEVE consultar este Codex.
...
| **Provider cache** | `.claude/plans/plan-{M}.md` ou `.cursor/plans/plan-{M}.md`, gitignored | AI working memory + scratch. Superset do body + blocos `<!-- not-flushed -->`. Nomeado pelo número da sub-issue | Cache local regenerável |
...
2. Nomear o arquivo como plan-{M}.md, onde {M} é o número da sub-issue
3. Confirmar via .gitignore que o diretório do provider está excluído
```

> **Modelo legado (deprecated):** arquivos `plan-{NNN}-{slug}.md` em `.claude/plans/` (sem sub-issue correspondente) são considerados zombies. Não criar arquivos novos sem sub-issue Plan aberta no GitHub. Caches existentes que não mapeiam para uma sub-issue devem ser triados em `.ahrena/issues/_legacy/` ou descartados.
...
Issue parent (#N) — User Story | Bug | Tech Task
    ...
    │   │
    │   ├── .claude/plans/plan-{M}.md ou .cursor/plans/plan-{M}.md  cache local
...
- `lex-agent-planning` — Lei correspondente (Gate 1 de `— → todo` + Gate 2 de `todo → development` + Tabelas A e B)
...
- `lex-issue-first`, `lex-issue-quality` — preconditions de criação de Issue parent e sub-issues Plan
- `lex-git-branches`, `lex-git-worktrees` — preconditions do Gate 2
- `lex-mcp` — preferência MCP + fallback CLI
- `kata-contributing-issue` — criação da Issue parent (precondition do Gate 1)
- `kata-decompose-issue-into-plans` — decomposição de Issue parent em sub-issues Plan
- `kata-plan-task` — criação de Plan avulso top-level vinculado a Issue existente
- `kata-load-plan-from-subissue` — materializa cache local do body canônico da sub-issue
- `kata-flush-plan-to-subissue` — flusha cache local (filtrando scratch) para o body da sub-issue
