...
> **Every agent MUST record a canonical plan as a GitHub sub-issue (Issue Type Task) linked to the parent Issue BEFORE starting any task that involves 2 or more steps, affects multiple files, or produces permanent artifacts. The plan MUST be presented to the user for confirmation before execution begins. Starting multi-step execution without a created and confirmed Plan sub-issue is FORBIDDEN. Materializing a local cache file (`.claude/plans/plan-{M}.md`, `.cursor/plans/plan-{M}.md`) without a corresponding open Plan sub-issue in GitHub is FORBIDDEN. The plan's `status:` lives as a **canonical label** on the sub-issue (and on the PR, starting at `to review`); the unified enum is `todo | development | to review | review | to release | release | done` (plus the alternate terminal `abandoned`); each transition MUST be performed by the owner declared in this Lex. The `— → todo` transition applies the creation gate (template + labels + Issue Type + Why/What/How); the `todo → development` transition applies the execution-start gate (remote branch + worktree + assignee).**
...
| **Provider cache** | `.claude/plans/plan-{M}.md` or `.cursor/plans/plan-{M}.md`, gitignored | AI working memory + scratch. Superset of the sub-issue body + `<!-- not-flushed -->` blocks. Named by the sub-issue number | Regenerable local cache |
...
The local cache is provider-specific: Claude agents use `.claude/plans/plan-{M}.md`; Cursor agents use `.cursor/plans/plan-{M}.md`. There is no shared cache between providers — each one carries its working memory independently, regenerated from the sub-issue via `kata-load-plan-from-subissue`.
...
Local cache schema (`.claude/plans/plan-{M}.md` or `.cursor/plans/plan-{M}.md`): **superset** of the sub-issue body. Carries the full mirrored body + marked local sections:
...
`kata-flush-plan-to-subissue` filters out `<!-- not-flushed -->` blocks before writing to the sub-issue body.
...
Materializing a local cache file `.claude/plans/plan-{M}.md` or `.cursor/plans/plan-{M}.md` without a corresponding open Plan sub-issue in GitHub is FORBIDDEN. When the user signals plan intent without referencing an Issue (e.g., "let's plan X"), the agent MUST:

1. Invoke `kata-contributing-issue` to open the parent Issue (User Story, Bug, or Tech Task as applicable).
2. Invoke `kata-decompose-issue-into-plans` to create the derived Plan sub-issues.
3. Only then invoke `kata-load-plan-from-subissue` to materialize the local cache.

`kata-load-plan-from-subissue` is the only canonical path to create a file in `.claude/plans/` or `.cursor/plans/` — and the kata MUST refuse if the Plan sub-issue does not exist in GitHub.
...
- **`.claude/plans/plan-{M}.md` or `.cursor/plans/plan-{M}.md`:** provider-specific regenerable local cache; never committed; rebuilt by `kata-load-plan-from-subissue` on fresh clone.
...
### Correct — Plan-first flow (intent without an Issue)
...
→ Agent creates file .claude/plans/plan-201.md as canonical before
  creating sub-issue #201 on GitHub
→ ❌ Violates the plan-first guardrail; local cache without a corresponding
  sub-issue is forbidden. Invoke kata-contributing-issue +
  kata-decompose-issue-into-plans before any local materialization
