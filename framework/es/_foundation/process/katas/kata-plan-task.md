...
| Template de Issue | No | `feature-request` (default), `simple-task`, `user-story-for-api`, `user-story-for-frontend` |
...
(3-layer: Issue body + .plans/ caché + .issues/ artifacts)
```
...
(working memory de la IA, gitignored) + .issues/{N}/ (committed Phase
artifacts).
