...
Main entry point to spawn a PoV (Proof of Value) for an agent on the Anthropic stack — Skill, Subagent, or Plugin. Invokes `warrior-claudionor` (Pre-operational Agent Factory), who orchestrates the 7 POV katas (`kata-pov-scope-define` → `kata-pov-value-track`) followed by implementation (skill via `kata-skill-implement`, subagent via `kata-agent-author`, plugin via plan-034). Produces `docs/{context}/agents-pov/` consumable by `cry-agent-design --from-pov` when the agent matures to `operational-concrete`.
