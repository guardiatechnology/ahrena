...
Criar ou atualizar conteúdo no Notion (páginas, blocos, propriedades de banco de dados) via servidor MCP. Cobre quatro operações: criar uma nova página, adicionar blocos a uma página existente, atualizar propriedades de uma página e excluir um bloco específico. Sempre verifica se o alvo existe antes de escrever e confirma ações destrutivas com o usuário.

## Quando Usar

- Quando o usuário precisa criar uma nova página ou documento no Notion
- Quando conteúdo deve ser adicionado a uma página existente (ex.: notas de reunião, sincronização de artefatos)
- Quando propriedades de uma entrada de banco de dados devem ser atualizadas (ex.: status, data, responsável)
- Quando um bloco específico deve ser removido de uma página
- Quando invocado por um Warrior que produz output destinado ao Notion (ex.: sincronização de documentação, catálogo de eventos, ADR)

## Inputs

| Input | Obrigatório | Descrição |
|-------|:-----------:|-----------|
| Operação | Sim | `create` (nova página) \| `append` (adicionar blocos) \| `update-props` (propriedades de página/entrada) \| `delete-block` (remover um bloco) |
| Alvo | Condicional | ID ou URL da página ou banco de dados pai para `create`; ID ou URL da página para `append` e `update-props`; ID do bloco para `delete-block` |
| Conteúdo | Condicional | Conteúdo de blocos para `create` e `append`; mapa de propriedades para `update-props` |
| Título | Condicional | Título da página — obrigatório para `create` |
| Tratamento de duplicatas | Não | `skip` (não faz nada se já existe página com o mesmo título), `update` (adiciona à página existente), `create-new` (sempre cria); padrão: `skip` |
...
- `codex-mcp-notion` — Referência de ferramentas e parâmetros do MCP Notion
- `kata-mcp-notion-read` — Kata para leitura de conteúdo Notion antes de escrever
