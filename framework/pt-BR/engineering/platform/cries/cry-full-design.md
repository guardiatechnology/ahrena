...
Este comando executa o **design completo** da superfície da feature: em uma única sequência, aciona o Warrior Daedalus para desenhar a API (OpenAPI + documento da API em **paths.oas**) e em seguida o Warrior Kronos para documentar os eventos (Markdown em **paths.events**). O agente executa as duas fases em sequência, usando a mesma descrição da feature como base. Equivale a combinar **cry-api-design** e **cry-event-storm** em um único fluxo.
...
1. **Fase 1 — API:** Assume o papel do Warrior Daedalus; executa **kata-api-design-oas** e **kata-api-design-doc**; produz especificação OpenAPI e documento da API em **paths.oas** (docs/oas)
2. **Fase 2 — Eventos:** Assume o papel do Warrior Kronos; executa **kata-events-doc**; produz documentação de eventos em **paths.events** (docs/events)
...
4. Entrega resumo dos artefatos produzidos: OAS e doc da API em paths.oas; doc de eventos em paths.events
...
1) **Fase API (Daedalus):** Atue como o Warrior Daedalus. Execute **kata-api-design-oas** e **kata-api-design-doc** com base na descrição da feature. Faça perguntas de clarificação se necessário (escopo, autenticação, paginação, base path). Produza especificação OpenAPI e documento da API em **paths.oas**.

2) **Fase Event Storm (Kronos):** Atue como o Warrior Kronos. Com base na mesma feature (e no contexto de eventos, se informado), execute **kata-events-doc**. Identifique os eventos relevantes (ex.: created, updated, cancelled para as operações da API), faça perguntas de clarificação se necessário, e produza a documentação de eventos em **paths.events**.

Entregue um resumo final: artefatos em paths.oas (OAS + doc da API) e em paths.events (doc de eventos).
```
...
- **Fase 1:** Recursos e endpoints (POST, GET, GET/:id, PATCH, DELETE); especificação OpenAPI e doc da API criados/atualizados em **paths.oas**
- **Fase 2:** Catálogo de eventos (event.guardia.platform.scheduled_transfer.created, .updated, .cancelled); doc de eventos criado/atualizado em **paths.events**
- Resumo: três artefatos — OAS e doc da API em docs/oas; doc de eventos em docs/events
