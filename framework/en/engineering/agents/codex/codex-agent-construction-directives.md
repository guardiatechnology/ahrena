...
Reference manual for architecting AI agents on the Guardia platform. Codifies in operational form the content of the manual "Diretrizes para Construção de Agentes" maintained in Notion (live source-of-truth). Companion to the Lex `lex-agent-construction-directives`; provides the conceptual detail, canonical examples, and differential rigor that the Lex references.

This Codex is consulted when building, reviewing, or promoting an agent — by humans, by `warrior-claudionor` (PoV Factory), by `warrior-metis` (APM Operational Concrete), by `warrior-apollo-agents` (implementation), and by `warrior-athena` (Gate 2 when the feature touches agents).

## Context

- **Domain:** construction of AI agents (system prompt, memory, tools, feedback, scope, context) and promotion of a PoV to Operational Concrete
- **Audience:** agent engineers, tech leads, product managers, AI agents that orchestrate construction
- **Update:** Notion is the live source; this Codex is the operational snapshot. Quarterly review; in case of divergence, **Notion prevails**.
...
Each DoOC item declared in the Lex has an expected evidence format:

| # | Item | Evidence format |
|---|------|-----------------|
| (a) | Declared PoV origin | Absolute link to `docs/{context}/agents-pov/{agent}/pov.md` |
| (b) | Proven leading metric | Number + threshold + window (e.g., `accuracy >= 0.92 over a 7-day window with n≥500 classifications`) |
| (c) | Declared lagging metric | Business metric + baseline (e.g., `monthly close time: baseline 14d, target 9d`) |
...
| (h) | Criticality tier | `tier-1` \| `tier-2` \| `tier-3` \| `tier-4`; tier-1/2 references the SLO under `docs/{context}/metrics/slo-{agent}.yaml` |
...
- **Notion (live source — prevails on divergence):** "Diretrizes para Construção de Agentes" (`35b36f91ebd281c8a65de122b7234b5d`)
- **Corresponding Lex:** `lex-agent-construction-directives`
- **Related Lexis:**
  - `lex-hard-gate-pattern` — HARD-GATE block format
  - `lex-slo-required` — mandatory SLO for tier-1/2
  - `lex-observability-required` — trace + metric + log per runtime surface
  - `lex-data-retention` — retention per memory layer
  - `lex-mcp` — correct use of MCP servers in tools
  - `lex-idempotency` — idempotency for state-modifying tools
- **Related Codex:**
  - `codex-ai-first-experience` — agentic UX, HITL for irreversibles
  - `codex-incident-response` — incident cycle when a production agent fails
- **Related Warriors (future delivery):**
  - `warrior-claudionor` (plan-031) — PoV Factory; enforces `stage: pre-operational`
  - `warrior-metis` (plan-032) — APM Operational Concrete; validates DoOC and promotes
  - `warrior-apollo-agents` (plan-013) — implements agents following this Codex
- **External:**
  - Piaget, J. (1936). *La naissance de l'intelligence chez l'enfant*
  - Vygotsky, L.S. (1978). *Mind in Society: The Development of Higher Psychological Processes*
  - OWASP LLM Top 10 (consulted by `codex-system-prompt` — future delivery)
