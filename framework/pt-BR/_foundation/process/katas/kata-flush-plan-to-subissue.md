...
> **Prefixo:** `kata-` | **Tipo:** Skill Repetível | **Escopo:** Sincronização do cache local provider-specific (`.claude/plans/plan-{M}.md` ou `.cursor/plans/plan-{M}.md`) para o body canônico da sub-issue Plan, conforme o modelo hierárquico de `lex-agent-planning`

## Objetivo

Persistir o conteúdo do cache local provider-specific (working memory da IA) no body da sub-issue Plan `{M}` no GitHub (canonical), filtrando blocos locais marcados `<!-- not-flushed -->` ... `<!-- /not-flushed -->`. Operação idempotente. Disparada nos 4 gatilhos canônicos de `lex-agent-planning`: cada transição de label `status:` na sub-issue/PR, cada Step concluído (`[ ]` → `[x]`), fim de sessão (heartbeat conclui ou owner sai), e handoff entre agentes.

## Quando Usar

- Transição de label `status:` na sub-issue Plan ou no PR vinculado (`todo → development`, `development → to review`, `to review → review`, etc.).
- Step do plano marcado como concluído (`[ ]` → `[x]`) no cache local.
- Fim de sessão Claude Code ou Cursor (heartbeat finaliza ou agente Athena/Argos/Janus sai).
- Handoff entre agentes (entrega antes do próximo agente entrar).
- Solicitação explícita do usuário ("flush plan", "atualiza a sub-issue").
...
| `source_path` | Não | Path do arquivo de cache local. Default: resolvido pela detecção de provider (`.claude/plans/plan-{M}.md` ou `.cursor/plans/plan-{M}.md`) |
| `force` | Não | `true` força gravação mesmo se houve edição remota desconhecida. Default: `false` (alerta + oferece merge manual) |
...
- `lex-agent-planning` — modelo hierárquico Issue → Plan → PR; cadência de flush (4 gatilhos canônicos)
- `lex-mcp` — preferência MCP + fallback CLI
- `lex-issue-status` — labels canônicas; flush é disparado em cada transição
- `codex-agent-planning` — manual operacional
- `kata-load-plan-from-subissue` — operação inversa (sub-issue → cache)
- `kata-plan-task` — criação da sub-issue Plan (precondition)
- `kata-decompose-issue-into-plans` — decomposição de Issue parent em N sub-issues Plan
- `kata-pr-prepare` — invoca este kata antes de abrir o PR
