...
| Stamp window | range `[branch_creation_date, mergedAt | now]` over which Claude Code sessions are considered for the calculation |
...
| Calendar time | wall-clock duration `[branch_creation_date, mergedAt | now]`; approximates lead time / throughput |
