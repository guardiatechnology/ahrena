...
Este Codex é a referência operacional para **incidentes em produção**. Define níveis de severidade, quem acionar em cada nível, como estruturar comunicação (interna e externa), quando rollback vs. forward-fix, e como conduzir post-mortem blameless. Consultado por `warrior-hestia` ao orquestrar uma resposta, por on-call engineers durante o incidente, e por liderança em comunicação com stakeholders.

## Contexto

- **Domínio:** incidentes em produção afetando serviços críticos (tier-1/2).
- **Público-alvo:** `warrior-hestia`, on-call engineers, incident commanders, stakeholders
- **Atualização:** após cada incidente sev-1 (lições aprendidas); trimestralmente (revisão de procedimento)
...
- `lex-slo-required` — SLO e error budget guiam priorização pós-incidente
- `lex-runbook-for-every-alert` — runbook é o passo 1 do diagnóstico
- `warrior-hestia` — conduz resposta
- `kata-incident-triage`, `kata-postmortem-write` — procedimentos detalhados
