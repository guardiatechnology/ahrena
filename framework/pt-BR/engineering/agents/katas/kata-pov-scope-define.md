...
Produzir, em `docs/{context}/agents-pov/{agent}/`, dois artefatos canônicos — `pov.md` (visão geral do PoV) e `scope.md` (escopo estabilizado, citado pela DoOC item d em `codex-agent-design-docs`) — com escopo **muito estreito** (1 caso de uso primário), critério explícito de descontinuação ("se em N semanas a métrica de valor não atingir X, encerra") e declaração explícita de `stage: pre-operational`. Aplica a Diretriz 05 de `lex-agent-construction-directives` (Escopo Restrito) na ótica de PoV: domínio estreito + feedback rápido = curva de aprendizado íngreme. Sem esses dois arquivos, nenhum outro kata do ciclo PoV pode rodar.

## Quando Usar

- Quando `cry-pov --context <name> --agent <slug> --kind <skill|subagent|plugin> --problem <description> --value-metric <description>` é invocado
...
| `--agent <slug>` | Sim | Slug do PoV em kebab-case (ex.: `rec-pov-classifier`). Define o subdir `docs/{context}/agents-pov/{agent}/`. Quando omitido em `cry-pov`, deriva-se de `{context}-pov` |
