...
| `graphify.auto_install_binary` | Quando `true` (default), `scripts/install.py` detecta se o binário `graphify` está no PATH e o instala quando ausente, via `uv tool install graphifyy` com fallback para `pipx install graphifyy`. Ambos isolam a ferramenta em ambiente próprio e não contaminam o venv do projeto. Falhas de instalação não são fatais — o grafo é insumo consultivo, então `kata-codebase-graph` degrada de forma limpa e nada quebra. Defina como `false` para pular a tentativa de instalação mantendo o contrato da diretriz. |
