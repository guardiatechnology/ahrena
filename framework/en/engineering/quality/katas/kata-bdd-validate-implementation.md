...
Produce `docs/issues/issue-{n}/08-bdd-validation-report.md` with the `SCN-{N}` ↔ existing-tests mapping, detect coverage gaps, verify the absence of a BDD step-runner in the manifests, and emit a `go | no-go` decision for Gate 3 (`kata-quality-gate` Check 8). **This Kata may read code** — that is exactly its purpose; the "blind to code" restriction only applies to scenario design (Phase 8.1).

## When to Use

- Phase 8.2 of the flow orchestrated by `warrior-athena`/`warrior-themis`, **after** `kata-bdd-scenarios-design` completes.
- On demand for BDD coverage audits on already-implemented features.
...
| `repo` | Yes | `owner/repo` |
| `07-bdd-scenarios.md` | Yes | Phase 8.1 artifact; fails if absent or malformed |
| Project manifests | Yes (in repo) | `pyproject.toml`, `package.json`, `go.mod`, etc. |
| Test suite | Yes (in repo) | `tests/`, `__tests__/`, `spec/`, stack equivalent |
...
- `lex-bdd-no-framework-coupling` — framework prohibitions and traceability rules
- `lex-bdd-gherkin-format` — format of parsed scenarios
- `codex-bdd` — principles and taxonomy
- `codex-gherkin` — expected syntax
- `codex-test-strategy` — level selection for gaps
- `lex-test-pyramid` — level distribution
- `kata-bdd-scenarios-design` — preceding stage (Phase 8.1)
- `kata-quality-gate` — Check 8 consumes this report's `gate_3_decision`
