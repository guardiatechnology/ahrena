...
O frontend é o primeiro alvo de ataques — roda no navegador do usuário, é visível para qualquer um com DevTools e recebe dados de fontes não confiáveis (URLs, inputs, APIs externas). Código frontend inseguro permite XSS (injeção de scripts no contexto do usuário), vazamento de tokens, CSRF e exposição de dados sensíveis. Agentes IA que geram frontend precisam tratar segurança como pré-requisito, não como item opcional.

Esta Lexis existe para garantir que **todo conteúdo dinâmico seja escapado**, que **credenciais e segredos nunca apareçam em código bundled**, que **headers de segurança sejam configurados** e que **inputs do usuário sejam validados no cliente** (além do servidor).
...
- **Aplica-se a:** todo código frontend que manipula dados dinâmicos, credenciais ou autenticação
- **Agentes vinculados:** `warrior-hephaestus`
- **Exceções:** Nenhuma. Lexis não admitem exceções.

## Consequências de Violação

1. **XSS:** atacante injeta JavaScript no contexto do usuário — pode roubar tokens, fazer ações em nome do usuário, exfiltrar dados
2. **Vazamento de credenciais:** secrets no bundle são extraídos em minutos; requerem rotação imediata
3. **CSRF:** ações não autorizadas executadas em nome do usuário autenticado
4. **Remediação:**
   - XSS: sanitizar todos os pontos de inserção; revisar código com DOMPurify ou equivalente
   - Segredos vazados: rotacionar imediatamente; auditar logs de acesso
...
- [OWASP Top 10 (com foco em A03 Injection, A07 Auth Failures)](https://owasp.org/www-project-top-ten/)
- [MDN — Content Security Policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP)
- `lex-mcp` — nunca hardcoded credentials
- `lex-python-security` — equivalente no backend
