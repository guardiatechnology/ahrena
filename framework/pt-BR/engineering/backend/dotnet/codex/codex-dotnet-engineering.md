...
Este Codex é o ponto de consulta do Apollo-.NET. Ele cobre decisões que atravessam C#, ASP.NET Core, EF Core, testes, resiliência, observabilidade e delivery, mantendo Clean Code e DDD como critérios de design e não como estruturas impostas.

## Contexto

- **Domínio:** aplicações e bibliotecas .NET modernas
- **Público-alvo:** Apollo-.NET, desenvolvedores e revisores de backend
- **Atualização:** a cada mudança do SDK suportado, target framework, analyzers, contratos operacionais ou ADR de arquitetura
- **Baseline local observado:** SDK 10.0.400 e runtime 10.0.11 em 2026-08-14; cada projeto deve declarar seu próprio `global.json`/TFM e não herdar essa versão por acidente
...
| Termo | Definição |
|---|---|
| TFM | Target Framework Moniker do projeto |
| Timeout budget | Tempo total repartido entre chamadas e tentativas |
| Expand/contract | Migração compatível em etapas para coexistência de versões |
| Commit incerto | Falha de comunicação em que não se sabe se a transação confirmou |
...
- `.references/TRILHA-DOTNET.md` e `.references/topicos/01-10`
- Documentação oficial catalogada em `.references/fontes/dotnet-oficial.md`
