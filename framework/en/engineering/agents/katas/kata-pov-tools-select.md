...
|-------|:--------:|-------------|
| `docs/{context}/agents-pov/overview.md` | Yes | Defines the primary use case |
...
```
Progress:
- [ ] 1. Read overview.md and list required capabilities
...
### Step 1: Read overview.md and list required capabilities
...
- Custom MCP servers (official Anthropic MCP servers are OK when already listed in `.ahrena/.directives::mcp.servers`)
...
If the primary use case **requires** something from the vetoed list, it is a strong signal that the PoV is premature: document the gap in `overview.md::Out of scope` and proceed without the tool.
...
Write `docs/{context}/agents-pov/tools.md` with sections: Required capabilities, Capability→tool mapping, Selected tools (one section per tool), Refused tools (with justification), Per-turn limits.
...
- Custom MCP for ERP: gap declared in overview.md::Out of scope
