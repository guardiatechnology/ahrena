...
Produzir, a partir de um projeto fonte validado, o pacote `.skill/` final consumido por agentes externos no formato Anthropic Agent Skills. O empacotador é determinístico: dada a mesma fonte e o mesmo commit do framework, o output é idêntico, com `.skill-manifest.json` ordenado lexicograficamente, todos os hashes SHA-256 conferidos e zero arquivos órfãos.

## Quando Usar

- Após `kata-skill-validate` retornar `ok` ou `ok-with-warnings`
- Quando `warrior-claudionor` precisa entregar o pacote final em `{paths.skills_dist}/`
- Durante a Phase 2 (build → dist) de uma release de skill
...
| `repo_root` | Não | Raiz do repositório (default: diretório de trabalho atual) |
| `skills_root` | Não | Override de `paths.skills_root` (default: lido de `.ahrena/.directives`) |
| `skills_build` | Não | Override de `paths.skills_build` |
| `skills_dist` | Não | Override de `paths.skills_dist` |
| `dry_run` | Não | `true` valida a fonte mas não escreve build/dist |
| `format` | Não | `text` (default, humano) ou `json` |
...
- `scripts/skills/package.py` — implementação determinística invocada
- `lex-skill-package-structure` — lei verificada (5 critérios + HARD-GATE)
- `lex-skill-project-structure` — pré-condição (fonte válida)
- `lex-semantic-version` — versionamento de `skill.version`
- `kata-skill-validate` — pré-requisito (mesmo runner)
- `warrior-claudionor` — orquestrador que invoca este kata
- `cry-skill` — atalho `--mode package`
