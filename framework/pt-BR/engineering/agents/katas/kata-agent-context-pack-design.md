...
Produzir o context pack do agent — few-shot positivos (mínimo 5 em produção, > que no PoV), exemplos negativos curados (mínimo 10), histórico observado dos últimos 30-90 dias quando aplicável. Cobre rigorosamente a **Diretriz 06 — Contexto Rico** de `lex-agent-construction-directives`.

Este Kata implementa a **ponte canônica `--from-pov`**: quando o agent vem com `entry_mode: with-pov`, lê `docs/{context}/agents-pov/{pov-agent}/` (output de `warrior-claudionor`) e enriquece o context pack com:

- Few-shot positivos derivados de inputs reais experimentados no PoV
- Exemplos negativos curados a partir de falhas observadas no PoV (anti-padrões reais, não inventados)
- Snippets de telemetria do PoV (traces típicos, edge cases observados)

## Quando Usar

- Após `kata-agent-feedback-design` (context pack referencia métricas e thresholds)
- Quando o agent recebe novo material de treinamento (re-curadoria periódica)
...
| `--from-pov <path>` | Não | `docs/{context}/agents-pov/{pov-agent}/` (output de `warrior-claudionor`) |
| `--retrain-cadence` | Não | `weekly` \| `monthly` \| `quarterly` (default: `quarterly`) |
...
| `value-proof.md` | `kata-pov-value-track` | Leading metric provada, decisão `pronto-para-DoOC` |
...
**Pressuposto de PII redaction:** o Kata confia que `kata-pov-value-track::Passo 4` aplicou o gate de PII grep antes de marcar `pronto-para-DoOC` (registrado em `value-proof.md`). Não revalida PII no input; documenta na seção `Validação de input em fronteira` o trust boundary.

Se o PoV ainda **não está** `pronto-para-DoOC` em `value-proof.md`, o Kata aborta com erro claro — context pack baseado em PoV imaturo viola o espírito da DoOC.
...
1. Se `--from-pov` fornecido:
   - Verifica que `pov_path/value-proof.md::status == pronto-para-DoOC` (ou equivalente). Falha se diferente
...
- **PII trust boundary:** confiamos no gate de PII grep aplicado por `kata-pov-value-track::Passo 4` no PoV de origem (quando `with-pov`). Este Kata não revalida PII; assume `pov_path/value-proof.md::status == pronto-para-DoOC` como prova de aprovação do gate
...
- `lex-agent-construction-directives::Diretriz 06`
- `system-prompt.md` — identidade que o context-pack reforça
- `guardrails.md` — categorias negativas alinhadas aos controles
- `feedback.md` — pivot trigger disparam re-curadoria
- `kata-system-prompt-adversarial-validate` — suite invocada quando negativos mudam
- `docs/{context}/agents-pov/{pov-agent}/` — fonte primária no modo `with-pov`
```
...
- PoV imaturo (`value-proof.md::status != pronto-para-DoOC`) como fonte é proibido — aborta o Kata
