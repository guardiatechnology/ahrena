...
> **Legacy model (deprecated):** files named `plan-{NNN}-{slug}.md` in `.claude/plans/` (without a corresponding sub-issue) are considered zombies. Do not create new files without an open Plan sub-issue in GitHub. Existing caches that do not map to a sub-issue must be triaged into `.ahrena/issues/_legacy/` or discarded.
