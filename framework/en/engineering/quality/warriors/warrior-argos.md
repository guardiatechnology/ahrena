...
> Take a Pull Request from a "diff plus checks" to a structured multi-axis review in a single command. Detect breaking changes the human eye misses, run the tests locally instead of trusting only CI, correlate the diff with the Issue, PRD, and Capability Spec, and consolidate everything into one idempotent review-comment that the human can then approve.
...
| `lex-pr-quality` | PR MUST mirror Issue labels, have size label, assignee, reviewers |
...
| `lex-git-worktrees` | Review runs inside a dedicated worktree |
| `lex-mcp` | Use MCP tools when listed in `mcp.servers`; surface choices on unavailability |
| `lex-issue-driven` | Multi-axis review reads `docs/issues/issue-{N}/` artifacts when present |
| `lex-pilars` | Cry → Warrior → Katas invocation chain (no Cry → Lexis/Codex) |
| `lex-cloudevents` | CloudEvents structure, `idempotencykey`, JSON < 12KB |
| `lex-restful-apis` | REST endpoint compliance (status codes, payload, headers) |
...
| `lex-python-typing`, `lex-python-error-handling`, `lex-python-result-type`, `lex-python-error-object` | Python compliance |
| `lex-frontend-typing`, `lex-frontend-accessibility`, `lex-frontend-security`, `lex-frontend-testing` | Frontend compliance |
| `lex-aws-iac`, `lex-aws-security`, `lex-aws-cost` | AWS infrastructure compliance |
...
| `lex-dry` | Domain knowledge MUST live in a single canonical locus per bounded context |
| `lex-test-pyramid`, `lex-test-isolation` | Test distribution and determinism |
| `lex-feature-design-docs` | `docs/{context}/{category}/` structure |

### Codex (Manuals it consults)

| Codex | Description |
|-------|-------------|
| `codex-issue-workflow` | Issue-Driven flow phases and artifacts |
...
| Kata | Description |
|------|-------------|
| `kata-mcp-github-read` | Read PR (view, diff, checks), linked Issue, comments via GitHub MCP |
| `kata-mcp-notion-read` | Read PRD and Capability Spec on Notion when linked from the Issue |
| `kata-git-worktree` | Create isolated worktree `.worktrees/review-pr-<N>/` |
...
| `kata-events-review` | CloudEvents review (symmetric pair of api-design-review) |
| `kata-security-review` | OWASP Top 10 + AuthN/AuthZ + sensitive data + dependencies |
| `kata-quality-gate` | When `docs/issues/issue-{N}/` exists, runs the 7 Gate 2 checks |
