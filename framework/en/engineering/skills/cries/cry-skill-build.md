...
Shortcut that invokes `kata-build-skill` for a project at `{paths.skills_root}/{slug}/`, running the deterministic pipeline (Validate → Build widgets → Freeze scripts → Resolve tools → Rewrite bindings → Emit) described in `codex-skill-build-pipeline`. The result is `{paths.skills_build}/{slug}/` + `{paths.skills_build}/{slug}.zip`, testable in another Claude Code agent before the final packaging in `.dist/` (PR 3).
...
- `kata-build-skill` — invoked procedure
- `codex-skill-build-pipeline` — pipeline contract
- `codex-skill-tools-and-widgets` — manifest schemas
- `lex-skill-project-structure` — source/build/dist separation
- `cry-skill-dev` — natural prior step (manual validation)
- `kata-package-skill` (PR 3) — consumer of `.build/` for `.dist/`
