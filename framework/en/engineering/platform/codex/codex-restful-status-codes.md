...
Allowed status codes and usage rules for HTTP endpoints on the Guardia platform. Codes used per endpoint MUST be documented in the OAS contract. Minimum standard for any Guardia RESTful API.

## Context

- **Domain:** HTTP status codes in API responses.
- **Target audience:** API implementers and consumers.
- **Update trigger:** when the Hub status codes specification changes.
