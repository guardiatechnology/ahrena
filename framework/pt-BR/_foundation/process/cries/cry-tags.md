...
Atalho do usuário que gerencia o objeto `tags` no heartbeat da sessão atual (`.ahrena/workflow/sessions/<id>.json`). Encapsula `kata-session-heartbeat` (para escritas) e `kata-session-tag-suggest` (para auto-inferência). O Cry nunca invoca Lexis ou Codex diretamente — roteia através das Katas conforme `lex-pilars`.

A auto-sugestão roda silenciosamente no primeiro turno do usuário quando `session_tracking.tags.auto_suggest: true`; `cry-tags` é a superfície manual de override.
