...
This Kata defines the procedure to **produce Markdown documentation** of CloudEvents for a feature or module: consult `lex-cloudevents`, `codex-cloudevents`, and `codex-feature-design-docs`; identify event types (format `event.guardia.{domain}.{entity_name}.{event_name}`); structure the content per entity with a `stateDiagram-v2` lifecycle and a CloudEvents payload per event; and delegate persistence to `kata-feature-design-docs` at `docs/{context}/events/events.md`.

## When to Use

- When a feature or module publishes or consumes events and those events need to be cataloged and documented
- When invoked by the Event Storm specialist Warrior (e.g., Kronos) or by `cry-event-storm`
- When you need to generate or update the events doc at `docs/{context}/events/events.md`

## Inputs

| Input | Required | Description |
|-------|:--------:|-------------|
| Feature or module context | Yes | Module name, entities involved, and operations that emit events (e.g., transaction.created, transaction.updated) or explicit list of event types |
| Bounded Context name | Yes | Bounded Context identifier in PascalCase (e.g., `ScheduledPayments`) |
| Base path / source | No | Base URI for `source` (e.g., `https://tenant.guardia.finance/platform/api/v1`). If omitted, the agent proposes per codex-cloudevents |
| Existing document | No | If an events doc exists at `docs/{context}/events/events.md`, update it instead of creating from scratch |
...
1. List **event types** in format `event.guardia.{domain}.{entity_name}.{event_name}` (e.g., `event.guardia.payments.transaction.created`, `event.guardia.payments.scheduled_transfer.cancelled`)
...
- event.guardia.payments.scheduled_transfer.created — after creation; source, subject, idempotencykey; data with entity_id, entity_type, created_at, updated_at, version, etc.
...
- `lex-feature-design-docs` — `docs/{context}/events/` structure
- `codex-feature-design-docs` — `events.md` template
- `kata-feature-design-docs` — canonical persistence
- `kata-events-review` — reviewing counterpart for PR-time CloudEvents review
