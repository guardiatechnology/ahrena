...
**Worktree-aware:** `auth.sh` resuelve `.env.local` y la caché del token desde la raíz del repositorio principal vía `git rev-parse --git-common-dir`, por lo que invocaciones desde cualquier `.worktrees/{N}-{slug}/` encuentran los mismos archivos del repositorio principal (sin duplicar credenciales ni regenerar tokens por worktree).
