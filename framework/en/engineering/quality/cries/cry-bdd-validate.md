...
Triggers the full Phase 8 for an Issue: `warrior-themis` produces `07-bdd-scenarios.md` (blind to code) and then `08-bdd-validation-report.md` (with test reading), closing with a `go | no-go` decision for Gate 3 in `kata-quality-gate`.

This Cry is one-to-one with `warrior-themis` (per `lex-pilars` Rule 5: Cry → 1 Warrior or 1 Kata). The Warrior internally orchestrates `kata-bdd-scenarios-design` and `kata-bdd-validate-implementation`.
...
- `warrior-themis` — invoked agent
- `kata-bdd-scenarios-design` — Phase 8.1
- `kata-bdd-validate-implementation` — Phase 8.2
- `lex-bdd-spec-only-sources`, `lex-bdd-gherkin-format`, `lex-bdd-no-framework-coupling` — BDD laws
- `lex-issue-driven` — Issue-Driven flow (Phase 8 and Gate 3)
- `codex-bdd`, `codex-gherkin` — reference manuals
