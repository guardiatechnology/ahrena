...
description: "Elicitação de Requisitos (perspectiva PO). Fase 2 do fluxo Issue-Driven — transformação do brief da Fase 1 em lista numerada de critérios de aceitação, DoD e out-of-scope"
---
...
- [ ] 6. Persistir em .ahrena/issues/{n}/02-requirements.md
- [ ] 7. Atualizar checkpoint
```

### Passo 1: Ler o brief da Fase 1

1. Ler `.ahrena/issues/{n}/01-brief.md`.
...
1. **Definition of Done** — checklist objetivo:
   ...
   - Gate 2 aprovado
   - Documentação em `.ahrena/issues/{n}/` completa
...
### Passo 6: Persistir em `.ahrena/issues/{n}/02-requirements.md`
...
- [ ] Documentação completa em `.ahrena/issues/{n}/`
...
1. Atualizar `.ahrena/workflow/issue-{n}/checkpoint.md` com:
   - fase concluída: 2
   - próxima fase: 3
   - referência: `.ahrena/issues/{n}/02-requirements.md`
   - número total de ACs
...
|-------|---------|---------|
| Documento de requisitos | Markdown com ACs numerados | `.ahrena/issues/{n}/02-requirements.md` |
...
- **Destino fixo:** `.ahrena/issues/{n}/02-requirements.md` (conforme `lex-issue-driven`).
