...
This Codex is the complete reference on how to write good commit messages following the Conventional Commits format, language rules, and Guardia best practices. It is consulted by `kata-commit` during message composition.

## Context

- **Domain:** Commit messages and versioning history
- **Target audience:** AI agents executing `kata-commit` and developers
- **Update:** When Guardia commit conventions change
...
| Term | Definition |
|------|-----------|
| Subject | First line of the commit: `type(scope): description` |
| Body | Detailed text after the subject, separated by a blank line |
| Footer | Metadata at the end: references, breaking changes, co-authors |
| Scope | Context in parentheses indicating the affected module or domain |
...
- `lex-conventional-commits` — Mandatory format law
- `lex-signed-commits` — GPG signature law
- `lex-small-commits` — Atomic commits law
- `lex-commit-language` — Commit language law
