...
AI sessions are expensive contexts: each context-switch destroys cache, fragments attention, and degrades execution quality. When a Plan is in `status: development` with the current session as the declared owner (assignee), accepting unrelated requests drains delivery velocity and dilutes the focus that the Ahrena framework optimizes.

The agent acts as the discipline-keeper against context-switching. It is not the user's job to remember to stay focused — it is the agent's job to politely decline and return to execution. This Lex codifies that decline as obligation, not courtesy.
