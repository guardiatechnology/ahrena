...
| Aspect | Cry | Kata |
|--------|-----|------|
| **Nature** | Entry point; decides which Kata to run based on target; invalid targets → message and list | Specific procedure per target (install, update, dev-install, bootstrap, sync-cursor, uninstall, clean) |
| **Parameters** | Minimal (target + optional variables) | Variables processed; consults codex-make |
| **Content** | No reference tables; only the dispatch table | Does not duplicate tables; refers to codex-make |
