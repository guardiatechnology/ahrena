...
`git-spice` é uma CLI escrita em Go (licença GPL-3.0) que automatiza operações de stacked branches em git nativo. O caso de uso central no contexto Ahrena é eliminar o cascade rebase manual: quando uma camada inferior recebe `commit create` ou `commit amend` via `git-spice`, todas as camadas superiores são automaticamente rebaseadas. Outras facilidades incluem trunk declarado uma única vez (`gs repo init --trunk`), `gs repo sync` que deleta branches mergeadas e rebaseia o restante, e `gs auth login` que reusa o token do `gh` via método CLI.

Este Codex é a referência de campo quando `.ahrena/.directives` declara `stacked_prs.tool: gs`. As decisões estratégicas (stack vs PR único, decomposição em camadas, naming, política bottom-up) continuam em `codex-stacked-prs` — este documento cobre apenas a operação com `gs`.

## Contexto

- **Domínio:** automação de cadeia de branches stacked via CLI dedicada; alternativa ao caminho vanilla (`git` + `gh`)
- **Público-alvo:** agentes de IA (warriors e katas que executam `kata-stacked-pr-create`, `kata-stacked-pr-rebase`, `kata-stacked-pr-merge`); contribuidores humanos que rodam stacks manualmente
- **Atualização:** quando flags do `gs` mudarem em release nova, quando uma versão do `gs` for adotada como mínimo testado pelo framework, ou quando troubleshooting recorrente justificar nova entrada
...
| Termo | Definição |
|-------|-----------|
| Trunk | Branch protegida contra writes, alvo final da stack (`main`, `master`, `release/*`). Declarada uma vez em `gs repo init --trunk` |
| Stack | Cadeia ordenada de branches onde cada uma tem a anterior como base; em git-spice, o conjunto rastreado pelo gs |
| Upstack / Downstack | Camadas acima / abaixo da branch atual na stack |
| Restack | Re-rebase de uma ou mais camadas contra suas bases atualizadas; em gs é automático após `commit create`/`amend` |
| Track | Importar uma branch git existente para o tracking do gs |
| CR (Change Request) | Termo agnóstico do gs para Pull Request (GitHub) / Merge Request (GitLab) / Pull Request (Bitbucket) |
| `--force-with-lease` | Variante de `git push --force` que recusa overwriting de commits remotos não fetchados localmente; default em `gs submit` |

## Referências

- [git-spice — site oficial](https://abhinav.github.io/git-spice/)
- [git-spice — repositório (GPL-3.0)](https://github.com/abhinav/git-spice)
- `codex-stacked-prs` — modelo conceitual e Decision Checklist canônica (compartilhada entre vanilla e gs)
- `kata-stacked-pr-create` seção "Variant: git-spice" — passos operacionais de criação
- `kata-stacked-pr-rebase` seção "Variant: git-spice" — auto-restack e resolução de conflitos
- `kata-stacked-pr-merge` seção "Variant: git-spice" — merge bottom-up com `gs repo sync`
- `cry-new-stacked-pr` — atalho que despacha kata-create por `stacked_prs.tool`
- `lex-protected-trunk` — proteção do trunk preservada (`gs` honra)
- `lex-git-branches` — `gs` aceita o naming `{type}/{N}-stack-{layer}-{slug}`
- `lex-signed-commits` — assinatura GPG preservada via `commit.gpgsign=true` global
- `lex-pr-quality` — labels/assignee/reviewers continuam aplicados via `gh pr edit` por PR
