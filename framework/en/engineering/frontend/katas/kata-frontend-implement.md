...
Given a frontend feature (component, page, interaction flow), produce a complete implementation in TypeScript with behavioral tests, accessibility, strict types, and adherence to the project's architectural patterns. Includes UI components, hooks/composables, API integration, state management, and tests.

## When to Use

- When a frontend feature has been described (brief + requirements) and needs to be implemented
- Invoked by `warrior-hephaestus` directly or via delegation from `warrior-athena` in Phase 4 of the Issue-Driven flow

## Inputs

| Input | Required | Description |
|-------|:--------:|-------------|
| Feature description | Yes | Requirements + acceptance criteria (numbered ACs if coming from the Issue-Driven flow) |
| API contract | No | OAS or request/response types (ideal: produced by `warrior-daedalus`) |
| Design or wireframe | No | Figma, images, or textual UI description |
| Detected stack | Yes | Framework (React/Next/Vue/Angular), language (TS), tools (testing, styling) |
...
- `codex-frontend-architecture` — structure and layer patterns
- `warrior-hephaestus` — agent that executes this kata
- `kata-api-design-oas` — API contract produced by Daedalus
