...
A scenario coupled to a UI selector becomes obsolete on the next redesign and stops describing behavior. A scenario with `status code 201` tests the protocol, not the business rule. BDD exists to protect **behavioral intent** (ubiquitous language), not to duplicate unit or contract tests with prose around them.

This Lexis exists so that **a scenario survives implementation refactors** and **communicates the expected behavior unambiguously to any business or engineering reader**.
...
1. **Gate 3 blocks:** `kata-quality-gate` Check 8 fails when the scenarios lint finds forbidden patterns or missing tags.
2. **Scenario discarded:** imperative scenarios are regenerated from the specification sources (per `lex-bdd-spec-only-sources`), not patched.
3. **Value erosion:** scenarios coupled to implementation age badly, become debt, and train the team to ignore them — blocking at the gate prevents the habit from setting in.
...
- `codex-gherkin` — Gherkin manual adopted at Guardia
