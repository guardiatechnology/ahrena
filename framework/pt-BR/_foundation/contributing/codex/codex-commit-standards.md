...
Este Codex é a referência completa sobre como escrever boas mensagens de commit seguindo o formato Conventional Commits, as regras de idioma e as boas práticas da Guardia. É consultado pelo `kata-commit` durante a composição de mensagens.

## Contexto

- **Domínio:** Mensagens de commit e histórico de versionamento
- **Público-alvo:** Agentes de IA executando `kata-commit` e desenvolvedores
- **Atualização:** Quando as convenções de commit da Guardia mudarem
...
| Termo | Definição |
|-------|-----------|
| Subject | Primeira linha do commit: `tipo(escopo): descrição` |
| Body | Texto detalhado após o subject, separado por linha em branco |
| Footer | Metadados no final: referências, breaking changes, co-authors |
| Escopo | Contexto entre parênteses que indica o módulo ou domínio afetado |
...
- `lex-conventional-commits` — Lei de formato obrigatório
- `lex-signed-commits` — Lei de assinatura GPG
- `lex-small-commits` — Lei de commits atômicos
- `lex-commit-language` — Lei de idioma de commits
