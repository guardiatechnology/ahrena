...
1. Compare the list of modified files (Step 1) with the Phase 3 component table. The boundary includes rows whose `Source` column is `graph (reverse)` — they are part of the declared scope like any other.
...
4. **Qualify the candidates with the graph** when `graphify.enabled` is `true`. Invoke `kata-codebase-graph` and check whether each file outside the table is a reverse consumer of a declared component:
   - **It is a reverse consumer** → likely a legitimate consequence of the change, not scope creep. Record it as `ripple` with the `file:line` origin and ask for human confirmation instead of blocking.
   - **It has no graph relationship to any declared component** → a stronger scope creep signal than mere absence from the table.
5. New public functions/classes in touched files that do not map to any AC → flag.

Check 2 result: ✅ if only declared files + exceptions were modified; ❌ if there is unjustified scope creep. Candidates the graph qualifies as `ripple` do not fail the check on their own — they become a question for the human.

#### Graph use in this check

- The graph **qualifies** candidates; it neither creates nor automatically absolves them. The human decides.
- Do not consult the graph for every file in the diff. Consult only the candidates from item 2, which are few by construction.
- Ignore findings that are only re-export barrels (`__init__.py`) — 47% of raw findings in the reference measurement were of that kind.
- A row sustained only by an `INFERRED` edge is never enough to classify something as `ripple`.

#### Degradation

When the binary is absent, `graphify.enabled` is `false`, or `built_at_commit` diverges from `HEAD`, the check proceeds exactly with the prior behavior (items 1 through 3 and 5) and **declares** that the graph was unavailable. No CI gate depends on the graph.
...
- `kata-codebase-graph` — qualification of scope creep candidates in Check 2
- `codex-graphify` — graph limits and degradation behavior
