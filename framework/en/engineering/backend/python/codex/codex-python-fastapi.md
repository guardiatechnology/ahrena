...
This manual defines the patterns for building FastAPI applications. FastAPI is the HTTP transport layer — it receives requests, validates input, delegates to use cases, and serializes responses. It MUST NOT contain business logic.

## Context

- **Domain:** FastAPI HTTP layer within Clean Architecture.
- **Target audience:** implementers and AI agents building FastAPI endpoints.
- **Update trigger:** when FastAPI patterns evolve or new middleware is adopted.
...
| Term | Definition |
|------|------------|
| Router | FastAPI `APIRouter` grouping related endpoints |
| Depends | FastAPI dependency injection mechanism |
| Schema | Pydantic model for request/response serialization |
