...
| `rtk.auto_install_binary` | Placeholder reservado (default `false`). A lógica de auto-install propriamente dita exige um ADR de continuidade — o RTK não possui auto-updater nativo e o canal de instalação varia por ambiente (`cargo install`, `brew install`, `curl \| sh`, npm). Quando `false` (default), install/update emite um warning se `command -v rtk` retornar falso, mas não instala. |
