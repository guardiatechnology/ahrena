...
Lexis frequentemente exigem bloqueio de fluxo: ações proibidas, pré-condições inegociáveis, gates que não admitem exceção. Sem sintaxe canônica, o `MUST` textual é fácil de ignorar — humanos racionalizam ("este caso é diferente"), agentes interpretam ambiguamente.

Esta Lexis estabelece o **bloco `<HARD-GATE>` literal** como sintaxe canônica para tornar bloqueios inequívocos. O bloco enumera explicitamente: (a) sujeito proibido, (b) ação no infinitivo, (c) pré-condições numeradas, (d) escopo de aplicabilidade, (e) anti-pretextos comuns, (f) exceções declaradas (ou "Nenhuma").

Inspirado no padrão [HARD-GATE textual](https://github.com/obra/superpowers) usado em skills do framework `obra/superpowers`.
...
1. **Detecção em revisão de PR:** revisor humano ou linter (a desenvolver) identifica cláusula de bloqueio sem bloco `<HARD-GATE>` correspondente
2. **PR rejeitado:** Lexis nova ou modificada com cláusula de bloqueio sem o pattern não passa pela revisão
3. **Remediação:** atualizar a Lexis aplicando o pattern nas 3 línguas suportadas (`pt-BR`, `en`, `es`); inclusão na próxima rodada de sync de `.cursor/` e `.claude/`
