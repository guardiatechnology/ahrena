...
User shortcut that invokes `kata-checkpoint-save` to write the `.checkpoint` of the current session. Useful when there is context outside the plan (Open threads, Notes, hand-off between multiple active plans) worth preserving before pausing or closing the window.

Reading `.checkpoint` is the agent's automatic responsibility at the start of the session (via `kata-checkpoint-read`) — `cry-checkpoint` covers only the on-demand **write** trigger.
