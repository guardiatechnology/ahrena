...
| `graphify.enabled` | When `true`, `scripts/install.py` enables Graphify (codebase knowledge graph) in the target project: it detects or installs the `graphify` binary and reports whether `graphify-mcp` is available. Unlike RTK, the default when the section is omitted is `false` — Graphify is an optional feature that pulls a large dependency tree (roughly 36 tree-sitter grammars), so an install that never asked for it stays clean. No hook is registered: the graph is queried on demand by `kata-codebase-graph`, never on every Bash call. See `codex-graphify`. |
| `graphify.auto_install_binary` | When `true` (default), `scripts/install.py` detects whether the `graphify` binary is on PATH and installs it when absent, via `uv tool install graphifyy` with a `pipx install graphifyy` fallback. Both isolate the tool in its own environment and do not pollute the project venv. Install failures are non-fatal — the graph is advisory input, so `kata-codebase-graph` degrades cleanly and nothing breaks. Set to `false` to skip the install attempt while keeping the directive contract. |
| `graphify.mode` | `code-only` (default) runs local AST extraction only: deterministic, no API key, no network. `semantic` enables the optional language-model pass over documents, PDFs, and images. No CI gate MUST depend on `semantic` mode. See the measured cost model in `codex-graphify`. |
