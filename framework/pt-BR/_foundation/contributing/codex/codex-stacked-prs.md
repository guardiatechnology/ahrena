...
Stacked Pull Requests são uma cadeia de PRs onde cada branch targeta a anterior, permitindo que uma feature grande seja decomposta em camadas reviewáveis em isolamento. Em vez de um único PR de 2 mil linhas que recebe rubber-stamp, são N PRs de 200-400 linhas cada um, encadeados, mergeados de baixo para cima.

Este Codex é o manual canônico para decidir **quando** stackar e **como** operar a cadeia. A operação descrita aqui usa exclusivamente `git` + `gh` CLI (caminho **vanilla**), funcionando em qualquer repositório GitHub. Caminhos com ferramentas (git-spice, gh-stack) reusam os conceitos definidos aqui e adicionam automação por cima.

A adoção é **opcional** e governada pela diretiva `stacked_prs.tool` em `.ahrena/.directives` (default `vanilla`).

## Contexto

- **Domínio:** decomposição de features grandes em PRs encadeados
- **Público-alvo:** todos os contribuidores (humanos e agentes); Athena no fluxo Issue-Driven (futuro)
- **Atualização:** quando Lexis referenciadas mudam, quando novos caminhos de tool entram (plan-005, plan-006), ou quando a Decision Checklist é refinada com base em uso real
...
| Pilars técnicos atravessados | ≥ 2 (ex.: backend + frontend) |
...
- `lex-issue-first` — issue obrigatória antes da branch
- `lex-issue-quality` — 5 critérios canônicos do issue
- `lex-git-branches` — naming pattern de branches
- `lex-git-worktrees` — exceção declarada para stacks
- `lex-pr-quality` — HARD-GATE aplicado por PR da stack
- `lex-protected-trunk` — proteção do trunk preservada
- `lex-conventional-commits`, `lex-small-commits`, `lex-signed-commits` — disciplinas de commit mantidas
- `kata-stacked-pr-create` — Pre-flight (Fase 0) + criação da cadeia
- `kata-stacked-pr-rebase` — cascade rebase manual
- `kata-stacked-pr-merge` — merge bottom-up
- `cry-new-stacked-pr` — atalho que invoca o kata-create
- `kata-contributing-pr` — fluxo de PR único (fallback quando Decision Checklist reprova)
