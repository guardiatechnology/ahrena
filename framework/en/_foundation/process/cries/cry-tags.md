...
User shortcut that manages the `tags` object in the current session's heartbeat (`.ahrena/workflow/sessions/<id>.json`). Wraps `kata-session-heartbeat` (for writes) and `kata-session-tag-suggest` (for auto-inference). The Cry never invokes Lexis or Codex directly — it routes through the Katas per `lex-pilars`.

Auto-suggestion runs silently on the first user turn when `session_tracking.tags.auto_suggest: true`; `cry-tags` is the manual override surface.
...
**Force auto-suggest re-inference:**
...
Output (stderr, no write):

```
ERROR: kind 'documentation' not in session_tracking.tags.kinds.
Configured kinds: tech-task, bug, spike, user-story, epic, chore, design, review, exploration, release.
```
