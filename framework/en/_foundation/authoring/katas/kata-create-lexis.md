...
This Kata defines the standardized procedure for creating a new Lexis in Ahrena — from conceiving the law to creating the artifact in all three mandatory languages.

## When to Use

- When it is necessary to establish an absolute constraint that no agent can violate
- When the user explicitly requests the creation of a new Lexis
- When invoked by `cry-new-lex`

## Inputs

| Input | Required | Description |
|-------|:--------:|-------------|
| Subject | Yes | Topic of the law (e.g., "mandatory code review", "no secrets in repository") |
| Scope | No | Where the law applies (e.g., "all repositories", "CI/CD pipeline"). If omitted, the agent MUST infer from the subject |
| Clade/Subclade | No | Where to save the artifact in the taxonomy. If omitted, the agent MUST infer from the subject |
...
- `codex-pilars` — Validation checklist for Lexis (Artifact validation section)
- `codex-lexis` — Quality criteria for Lexis
...
- `templates/lex-sample.md` — Official template
