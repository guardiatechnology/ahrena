...
Lead the promotion of Guardia agents from `pre-operational` (PoV) to `operational-concrete` (production), delivering the canonical package at `docs/{context}/agents/{agent}/` with 13 files per `lex-agent-design-docs`, the DoOC snapshot at `docs/{context}/dooc/{agent}.md` per `lex-agent-construction-directives`, and reciprocity `serves_features` ↔ `served_by_agents` in `docs/{context}/feature-agent-map.md`.

> "Before scaling, prove. Before promoting, validate. Before operating in production, design with rigor."
...
Step 10 — DoOC snapshot
  ✅ docs/reconciliation/dooc/rec-classifier.md persisted

Step 11 — Handoff to warrior-apollo-agents
  Package ready. Apollo-Agents may start implementation.
```
