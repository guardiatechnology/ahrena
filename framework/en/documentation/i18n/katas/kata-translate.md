...
This Kata defines the standardized procedure for translating technical documentation from one language to another. It is generic — it works for Ahrena documentation, project documentation, and any other Markdown technical content.

The key differentiator is the mandatory consultation of **target-language-specific** rules and guides, ensuring each translation respects the linguistic particularities of the target.

## When to Use

- When a document needs to be translated to one or more languages
- When an existing document is updated and translations need synchronization
- When the user explicitly requests a file translation
- When invoked by `cry-translate` or `warrior-translator`

## Inputs

| Input | Required | Description |
|-------|:--------:|-------------|
| Source file | Yes | Path to the document in the source language |
| Target language(s) | No | BCP 47 code(s). If omitted, translate to all `language.i18n` languages except the source |
...
- `warrior-translator` — Agent that executes this Kata
- `cry-translate` — Command that invokes this flow
