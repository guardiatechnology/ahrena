...
This Kata defines the procedure to **produce Markdown documentation** of CloudEvents for a feature or module: consult lex-cloudevents and codex-cloudevents, identify event types (format `event.guardia.{module}.{entity_type}.{event_name}`), document structure, payload (data), idempotency, and persist the document in **paths.events** in compliance with Guardia rules.
...
- When you need to generate or update the events doc in paths.events (e.g., `events.md`, `cloudevents.md`)
...
| Existing document | No | If an events doc exists in paths.events, update it instead of creating from scratch |
