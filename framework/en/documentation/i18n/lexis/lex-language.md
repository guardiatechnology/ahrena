...
This Lexis defines the universal rules governing any translation, regardless of the target language. These are cross-cutting rules that apply **before** language-specific rules (`lex-language-ptbr`, `lex-language-en`, `lex-language-es`).

The goal is to ensure that every translation preserves fidelity to the original content, the document's structural integrity, and terminological consistency.
...
- **Applies to:** all technical documentation translation
- **Bound agents:** `warrior-translator` and any agent performing translation
- **Exceptions:** None. Lexis do not admit exceptions.

## Consequences of Violation

1. **Incomplete translation:** a translation with missing sections or source-language fragments is rejected.
2. **Loss of fidelity:** a translation that alters the original meaning compromises reliability.
3. **Remediation:** the agent must redo the translation, consulting this Lexis and the target language's `lex-language-{lang}`.

## References

- `lex-language-ptbr` — Rules for translating to Brazilian Portuguese
- `lex-language-en` — Rules for translating to English
- `lex-language-es` — Rules for translating to Spanish
- `codex-language` — Cross-cutting translation guide
- `kata-translate` — Translation coordination procedure
