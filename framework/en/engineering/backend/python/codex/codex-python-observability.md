...
Traces and metrics in Python backend. Logs live in `codex-python-logging`. Same principle: decorator-driven instrumentation, never inline. OpenTelemetry is the abstraction — product code does not know the backend (Prometheus, Datadog, OTLP). A single `@observe` decorator combines log + metric + span when it makes sense.

## Context

- **Domain:** telemetry (traces + metrics) in Python services — APIs, workers, jobs.
- **Target audience:** implementers and AI agents (warrior-apollo).
- **Update trigger:** OpenTelemetry stack evolution, backend change, new semantic conventions.
...
| Trace | End-to-end request path, composed of spans |
| OTLP | OpenTelemetry protocol for exporting telemetry |
| Result | `Ok[T] | Err[E]` type — replaces exception with return value |
| Outcome | `ok` (Ok), `failure` (Err), `error` (exception) |
| Cardinality | Number of distinct values for a label; high = expensive |
