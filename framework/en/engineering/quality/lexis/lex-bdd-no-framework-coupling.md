...
Step-runners (behave, pytest-bdd, cucumber, and the like) create a parallel test infrastructure (step definitions, regex matchers, hooks) that drifts from the real suite, doubles maintenance cost, and trains the team to chase glue rather than behavior. The promised benefit — "the business reads the tests" — rarely pays the cost: in practice, no business stakeholder reads the running `.feature`, and the team ends up fixing regex in `steps/` files.

This Lexis exists so that **Gherkin scenarios remain black-box documentation** (per `lex-bdd-spec-only-sources` and `lex-bdd-gherkin-format`) and **the test suite remains the test suite** — single, standard, with explicit traceability to the scenario via a naming convention.
...
1. **Gate 3 fails:** `kata-quality-gate` Check 8 detects a step-runner dependency or an executed `features/` directory and blocks the PR.
2. **Untraceable test:** a test without a matching `SCN-{N}` reference does not count as BDD coverage; the scenario becomes a gap in `08-bdd-validation-report.md`.
3. **Visible maintenance debt:** the removal ADR keeps the debt explicit until migration completes.
...
- `lex-bdd-spec-only-sources` — allowed sources for deriving scenarios
- `lex-bdd-gherkin-format` — declarative format for scenarios
- `lex-test-pyramid` — test level distribution
- `lex-test-isolation` — test independence
- `kata-bdd-validate-implementation` — scenario↔test validation procedure
- `warrior-themis` — agent that orchestrates the validation
