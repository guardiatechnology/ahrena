...
Esta Kata define o procedimento para deixar uma aplicação Python pronta para registrar logs conforme a `lex-logging-decorator`: instalar `loguru`, criar o módulo de bootstrap, implementar o decorator `@logged`, integrar com OpenTelemetry e FastAPI, ativar lint que bloqueia chamadas inline, e validar que tudo funciona ponta a ponta.

## When to Use

- Em um serviço Python que ainda não adotou o padrão de logging (sem `loguru` configurado ou usando `logging`/`structlog`/`print` no corpo do código).
- Ao iniciar um novo serviço Python a partir do template padrão.
- Ao remediar uma violação detectada em PR review (chamada inline ou import direto de logger).
- Quando invocado por `cry-python-implement` com escopo de observabilidade ou pelo Warrior Apollo.

## Inputs

| Input | Obrigatório | Descrição |
|-------|:-----------:|-----------|
| Caminho do pacote raiz da aplicação | Sim | Ex.: `app/`, `src/myservice/` |
| Nome do serviço | Sim | Identificador estável (ex.: `transfer-api`); usado em `service` no log |
| Nível de log default | Não | `INFO` por padrão; `DEBUG` apenas em ambientes não-prod |
| Lista de campos sensíveis adicionais | Não | Complementa a denylist global do sink (ex.: `card_number`, `tax_id`) |
...
- codex-python-logging (engineering/backend) — manual de implementação Python
