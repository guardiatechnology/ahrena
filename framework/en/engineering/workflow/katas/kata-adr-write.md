...
Write an individual Architecture Decision Record (ADR) in the simplified MADR format, with sequential numbering and status managed across the lifecycle (proposed → accepted → deprecated/superseded). Invoked by `kata-architecture-brief` for each relevant architectural decision identified, or manually when a decision needs to be formalized outside the Issue-Driven flow.

## When to Use

- Called by `kata-architecture-brief` in Phase 3 of the Issue-Driven flow for each relevant decision
- Called manually when an architectural decision needs to be formalized (e.g., framework migration, pattern adopted in the codebase)
- **Do not use** for: localized bugfixes, local refactors, endpoints following an existing pattern (per the checklist in `codex-issue-workflow`)

## Inputs

| Input | Required | Description |
|-------|:--------:|-------------|
| Title | Yes | Short imperative decision title (e.g., "Use FastAPI routers for module separation") |
| Context | Yes | Problem or force that motivated the decision |
| Decision | Yes | The decision made, in active voice |
| Alternatives considered | Yes | At least 1 alternative with rejection rationale |
| Initial status | No | `proposed` (default), `accepted` if approved in the same session |
| Related issue | No | GitHub issue number (e.g., `42`) |
...
- `lex-issue-driven` — mandatory ADR for relevant decisions
- `kata-architecture-brief` — kata that invokes this one
