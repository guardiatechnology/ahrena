...
Escrever um Architecture Decision Record (ADR) individual no formato MADR simplificado, com numeração sequencial e status gerenciado ao longo do ciclo de vida (proposed → accepted → deprecated/superseded). Invocado por `kata-architecture-brief` para cada decisão arquitetural relevante identificada, ou manualmente quando uma decisão precisa ser formalizada fora do fluxo Issue-Driven.

## Quando Usar

- Chamado por `kata-architecture-brief` na Fase 3 do fluxo Issue-Driven para cada decisão relevante
- Chamado manualmente quando uma decisão arquitetural precisa ser formalizada (ex.: migração de framework, padrão adotado no codebase)
- **Não usar** para: bugfix pontual, refactor local, endpoint seguindo padrão (conforme checklist em `codex-issue-workflow`)

## Inputs

| Input | Obrigatório | Descrição |
|-------|:-----------:|-----------|
| Título | Sim | Título curto e imperativo da decisão (ex.: "Use FastAPI routers for module separation") |
| Contexto | Sim | Problema ou força que motivou a decisão |
| Decisão | Sim | A decisão tomada, em voz ativa |
| Alternativas consideradas | Sim | Pelo menos 1 alternativa com justificativa de rejeição |
| Status inicial | Não | `proposed` (padrão), `accepted` se aprovado na mesma sessão |
| Issue relacionada | Não | Número da issue do GitHub (ex.: `42`) |
...
- `lex-issue-driven` — obrigatoriedade de ADR para decisões relevantes
