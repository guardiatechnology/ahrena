...
| `bot_author.enabled` | Chave mestra para a identidade de autor de commit/PR do bot Ahrena. Quando `true`, os warriors listados em `bot_author.apply_to` chamam `scripts/ahrena-auth.sh` antes de `git commit` / `gh pr create`, de modo que commits e PRs sejam atribuídos à identidade do GitHub App `ahrena-bot[bot]` (assinada no servidor pelo token de instalação do App) e o ser humano que conduziu o trabalho seja registrado como `Co-authored-by:`. Default `false` — o comportamento atual de autoria humana é preservado bit por bit até que o projeto opte explicitamente. Ver `codex-git-workflow` ("Author identity"). |
| `bot_author.identity` | Slug do GitHub App usado para derivar a identidade do bot (default `ahrena-bot`). Sobrescreva apenas quando um fork ou clone utilizar outro slug. |
| `bot_author.commit_mode` | `api` (assinatura no servidor via token de instalação do App) — único modo disponível hoje. O valor `local` está reservado para uma iteração futura. |
| `bot_author.commit_co_author` | `human` (injeta `Co-authored-by: <nome humano> <email humano>` para que a pessoa que conduziu o trabalho permaneça rastreável) ou `none`. |
| `bot_author.apply_to` | Lista de nomes de warriors que respeitam o override. Warriors omitidos da lista mantêm o autor humano mesmo quando a chave mestra está ativada (opt-out por warrior). |
