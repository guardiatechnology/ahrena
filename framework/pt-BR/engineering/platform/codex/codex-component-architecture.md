...
Este Codex é o manual canônico da arquitetura por **components** que todo bounded context da plataforma Guardia adota. Define a estrutura física de pastas (`components/{api,agents,jobs,ui}/`, `deployment/`, `docs/`), as fronteiras entre os 5 components produtivos e os artefatos cruzados que cada bounded context produz. É a porta de entrada para os 5 codex-filhos especializados, que detalham as convenções internas de cada component.

O ponto de partida externo é o repositório de referência apontado em `references.component_template_repo.url` no `.ahrena/.directives` (per `lex-directives` e `codex-directives`). Codex nunca cita o repo pelo nome — sempre via diretiva. Quando o template externo evoluir, a diretiva é atualizada e os codex referenciados permanecem estáveis.

## Contexto

- **Domínio:** organização física de um bounded context da plataforma Guardia
- **Público-alvo:** engenheiros que iniciam um novo bounded context, agentes que delegam implementação por component (Athena, Apollo, Hephaestus, Iris, Métis), revisores de PR
- **Atualização:** quando o repositório de referência ganha um novo component produtivo, quando uma fronteira é redefinida via ADR
...
| Termo | Definição |
|-------|-----------|
| Component | Unidade física de código com manifest, testes e deploy próprios dentro do bounded context. 5 tipos canônicos: `api`, `agents`, `jobs`, `ui` (produtivos) + `deployment` (IaC) |
| Bounded Context | Termo DDD — fronteira semântica do domínio. No layout físico Guardia, cada bounded context é um diretório top-level com `components/`, `deployment/` e `docs/` |
| Orchestrator | Agent LLM principal de um bounded context, responsável por decompor tarefas para Specialists. Detalhado em `codex-component-agents` |
| Specialist | Agent LLM focado, invocado pelo Orchestrator para subtarefa específica. Detalhado em `codex-component-agents` |
| Widget | Componente React/Next.js exportável (via tsup) consumido por outros frontends Guardia. Detalhado em `codex-component-ui` |

## Referências

- `.ahrena/.directives` § `references.component_template_repo.url` — source-of-truth externa
- `codex-directives`, `lex-directives` — diretiva e sua aplicação
- `codex-component-api` — convenções de `components/api/`
- `codex-component-agents` — convenções de `components/agents/`
- `codex-component-jobs` — convenções de `components/jobs/`
- `codex-component-ui` — convenções de `components/ui/`
- `codex-component-deployment` — convenções de `deployment/`
- `lex-feature-design-docs`, `codex-feature-design-docs` — estrutura de `docs/{context}/`
- `lex-cloudevents`, `lex-idempotency` — comunicação cross-component via eventos
