...
This Codex is the canonical manual for the artifacts produced during the Ahrena Product Discovery phase. It defines the YAML front-matter schema for **insights** (produced by `warrior-pitia`) and **ideas** (produced by `warrior-phanes`), the state machine that governs the insight lifecycle, the numbering convention, and the canonical addressing inside `docs/discovery/`. The corresponding law is in `lex-discovery-flow`.

Insights are structured observations extracted from sources (APIs, docs, processes, interviews, screens) that describe opportunities, pains, or hypotheses about a business domain. Ideas are solution candidates: approved insights synthesized into problem, hypothesis, target user, success metric, and effort estimate. The insight → Idea pair forms the input of the design cycle (`warrior-prometheus` onward).

## Context

- **Domain:** Product Discovery — phase prior to the Ahrena design cycle
- **Audience:** `warrior-pitia`, `warrior-phanes`, Product Managers, stakeholders who evaluate insights, human authors
- **Update:** after each complete Discovery → Idea cycle (mandatory review after the first real use, recorded in an ADR when the schema changes)
...
| Term | Definition |
|------|------------|
| Topic | Theme of a Discovery initiative; aggregator of related insights and ideas |
| Insight | Structured observation extracted from sources; unit of discovery |
| Idea | Solution candidate derived from approved insights; unit of proposition |
| Promotion | `approved → promoted` transition of an insight, executed by `warrior-phanes` when generating the Idea |
| Refining | State in which `warrior-pitia` is iterating the insight after human feedback |

## References

- `lex-discovery-flow` — corresponding law with HARD-GATEs
- `kata-discovery-synthesis` — insight production procedure by `warrior-pitia`
- `kata-ideation-from-insight` — promotion procedure from insight to Idea by `warrior-phanes`
- `warrior-pitia`, `warrior-phanes` — bound agents
- `lex-feature-design-docs`, `codex-feature-design-docs` — downstream destination after Discovery (Prometheus consumes Ideas)
