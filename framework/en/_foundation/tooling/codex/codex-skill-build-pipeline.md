...
The build is the bridge between **source** (`skills/{slug}/`, authored with the Ahrena Pilars) and **delivery** (`.dist/{slug}.skill`, Anthropic format). It:

1. Validates `SKILL.md`, `skill.config.json`, the tools and widgets manifests
2. Builds React widgets (Vite) with configurable source maps
3. Freezes scripts (dependency lock) without installing the target runtime
4. Validates `tools/mcp.config.json` and resolves handler refs
5. Rewrites paths and bindings for the post-build format
6. Emits `.build/{slug}/` with a layout ready for packaging (PR 3) and `.build/{slug}.zip` manually testable in another agent

This codex documents the **contract** of the pipeline. The operational implementation lives in `kata-build-skill`. The final packaging in `.dist/` (snapshot of external refs with commit hash, root manifest with hashes) is covered in PR 3 (`kata-package-skill`, `lex-skill-export-determinism`).

## Context

- **Domain:** transformation source → intermediate (`.build/`); precondition for packaging
- **Audience:** `kata-skill-dev-server`, `kata-build-skill`; authors who need to understand what to expect from the build
- **Update:** when tooling defaults (Vite, uv, Node) change; when the structure of `.skill-manifest.json` evolves
...
| Term | Definition |
|------|------------|
| Phase | Named stage of the pipeline (Validate, Build widgets, Freeze scripts, Resolve tools, Rewrite bindings, Emit) |
| Freeze | Copy of `scripts/` with the lockfile preserved; without installing dependencies in `.build/` |
| Tool stub | Local mock of the tool, used by the dev server; does **not** go to `.build/` |
| Incremental pipeline | Re-execution that takes advantage of the Vite cache when valid |

## References

- `codex-skill-anthropic-agent-skills` — `SKILL.md` schema validated in Phase 1
- `codex-skill-tools-and-widgets` — schemas of `mcp.config.json` and `manifest.json`
- `codex-skill-project-architecture` — high-level dev → build → dist flow
- `codex-python-tooling` — uv as the target Python runtime
- `codex-frontend-architecture` — restrictions applicable to widgets
- `lex-skill-project-structure` — law that separates source/build/dist
- `lex-skill-export-determinism` (PR 3) — determinism of the final delivery
- `kata-skill-dev-server` — dev server orchestration that precedes the build
- `kata-build-skill` — operational implementation of this pipeline
