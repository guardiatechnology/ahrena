...
Testes de frontend têm um propósito específico: garantir que a UI **se comporta** como o usuário espera, em todos os fluxos relevantes. Testes que verificam detalhes de implementação (estado interno, snapshots sem revisão, chamadas de método) dão falsa segurança e quebram a cada refactor. Testes que verificam comportamento (o que o usuário vê e pode fazer) sobrevivem a refactors e capturam regressões reais.

Esta Lexis existe para garantir que **todo componente com lógica de negócio ou interação tenha testes**, que **testes sejam comportamentais (user-centric)** e que **mocks sejam usados apenas nas fronteiras externas** (API, Date, crypto, storage).
...
- **Aplica-se a:** todo código frontend com lógica de negócio ou interação (componentes, hooks, stores)
- **Agentes vinculados:** `warrior-hephaestus`
- **Exceções:** componentes puramente decorativos (ex.: `<Divider />`, ícones) podem não exigir teste — documentar a decisão

## Consequências de Violação

1. **Testes frágeis:** asserções em implementação quebram a cada refactor sem detectar regressão real
2. **Falsa segurança:** coverage alto com testes de implementação não previne bugs de usuário
3. **Lentidão de evolução:** cada mudança de UI requer reescrever testes
4. **Remediação:** reescrever testes no estilo user-centric; preferir queries semânticas; reduzir mocks internos
...
- `codex-frontend-architecture` — onde testar cada camada
- `lex-frontend-accessibility` — queries acessíveis exigem UI acessível
