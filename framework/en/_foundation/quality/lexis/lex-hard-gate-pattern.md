...
Lexis frequently require flow blocks: forbidden actions, non-negotiable preconditions, gates that admit no exception. Without canonical syntax, the textual `MUST` is easy to ignore — humans rationalize ("this case is different"), agents interpret ambiguously.

This Lexis establishes the **literal `<HARD-GATE>` block** as canonical syntax to make blocks unambiguous. The block explicitly enumerates: (a) forbidden subject, (b) action in infinitive, (c) numbered preconditions, (d) applicability scope, (e) common counter-pretexts, (f) declared exceptions (or "None").

Inspired by the [HARD-GATE textual pattern](https://github.com/obra/superpowers) used in skills of the `obra/superpowers` framework.
...
1. **Detection in PR review:** human reviewer or linter (to be developed) identifies blocking clause without corresponding `<HARD-GATE>` block
2. **PR rejected:** new or modified Lexis with blocking clause without the pattern does not pass review
3. **Remediation:** update the Lexis applying the pattern in the 3 supported languages (`pt-BR`, `en`, `es`); inclusion in the next round of `.cursor/` and `.claude/` sync
