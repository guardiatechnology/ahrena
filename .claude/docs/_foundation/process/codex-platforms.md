...
With `--platform codex`, Ahrena maps its pillars to native OpenAI Codex resources:

| Ahrena pillar | OpenAI Codex resource |
|---|---|
| Lexis | `.codex/docs/lex/` plus a managed operating contract in `AGENTS.md` |
| Codex | `.codex/docs/codex/` for progressive reference loading |
| Katas | `.agents/skills/<name>/SKILL.md` |
| Warriors | `.codex/agents/<name>.toml` |
| Cries | `.agents/skills/<name>/SKILL.md` |

Here, **Ahrena Codex** means a reference manual, while **OpenAI Codex** means the agent platform. Cries become skills because custom prompts are personal and skills are the recommended shareable mechanism. The installer preserves project content outside managed markers in `AGENTS.md` and `.codex/config.toml`. Human gates remain mandatory, while external actions (Issues, PRs, messages, and cloud resources) respect OpenAI Codex authorization boundaries.

Run `python .ahrena/update.py --sync-codex` or `make sync-codex` to regenerate the projection.
