...
Produzir ou atualizar os documentos de design de uma feature na estrutura canônica `docs/{context}/{categoria}/` definida por `lex-feature-design-docs` e detalhada em `codex-feature-design-docs`. Este Kata é o procedimento operacional consumido por `warrior-theseus` (entidades), `warrior-daedalus` (OAS) e `warrior-kronos` (eventos), invocado direta ou indiretamente pelo `warrior-prometheus` em cada fase do ciclo de design.

## Quando Usar

- Em qualquer fase do ciclo de design de feature orquestrado por `warrior-prometheus` em que um artefato de design (entidades, OAS ou eventos) precisa ser persistido
- Quando `warrior-theseus`, `warrior-daedalus` ou `warrior-kronos` precisa gravar ou atualizar seu output
- Quando uma feature em manutenção requer revisão da estrutura de documentos
- Quando se inicia um novo Bounded Context e a estrutura de pastas precisa ser criada do zero

## Inputs

| Input | Obrigatório | Descrição |
|-------|:-----------:|-----------|
| Bounded Context | Sim | Nome do contexto em PascalCase (será convertido para kebab-case). Ex.: `ScheduledPayments` |
