...
Standalone shortcut to invoke `kata-bdd-create-scenarios`. Reads a GitHub issue (and Notion context when MCP is configured), produces business-focused Gherkin scenarios, and persists them into the issue body inside the `bdd:scenarios` markers. Never reads source code. Independent of `/cry-implement-issue` — can be invoked before, after, or fully outside the Issue-Driven flow.
...
| Aspect | Cry | Kata |
|---|---|---|
| Nature | Quick invocation by issue number | Full procedure (read, draft, validate, confirm, persist) |
| Complexity | Low | High (9 steps incl. MCP, language validation, idempotent block update) |
...
- `lex-bdd-scenarios`, `lex-bdd-coverage` — laws
- `codex-bdd` — methodology
