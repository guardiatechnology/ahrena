...
This Codex is the central reference on the Ahrena Pilar system. It describes the nature of each Pilar, how they relate to each other, how to validate artifacts, and how the framework uses its own artifacts to evolve — the concept of self-sufficiency. It operationalizes `lex-pilars` with verifiable criteria and validation checklists per Pilar. For detailed documentation of each Pilar (how to write well, quality criteria), consult the specific Codex: `codex-lexis`, `codex-codex`, `codex-katas`, `codex-warriors`, `codex-cries`.

## Context

- **Domain:** Taxonomy, architecture, and validation of the Ahrena framework
- **Audience:** AI agents that create or validate artifacts; framework maintainers; PR reviewers
- **Update:** Whenever a new Pilar is created, relationships between Pilars change, or `lex-pilars` is updated
...
| Pilar validation | Verification that an artifact satisfies the definition and criteria of the Pilar it belongs to |
| Invocation chain | Sequence Cry → (Warrior) → Kata; Lexis and Codex are consulted, not invoked by the Cry |
| Canonical definition | Definition established in `lex-pilars` and operationalized in this Codex |

## References

- `lex-pilars` — Law that canonically defines the five Pilars and invocation rules (source of truth for validation)
- `.ahrena/.directives` — Canonical framework directives (paths, naming.prefixes)
- `lex-template-usage` — Mandatory template usage law
- `lex-framework-language` — Language structure law
- `codex-lexis`, `codex-codex`, `codex-katas`, `codex-warriors`, `codex-cries` — Detailed documentation of each Pilar (how to write well, quality criteria)
