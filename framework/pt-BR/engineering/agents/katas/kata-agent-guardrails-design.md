...
Produzir os três arquivos canônicos de controles do agent:

- `guardrails.md` — controles OWASP LLM Top 10 2025 críticos aplicados (referência `lex-system-prompt`); PII redaction at I/O boundary; isolamento `org_id`/`client_id`
- `authorization.md` — quem (humano ou outro agent) pode invocar este agent; quais escopos de cliente
- `escalation.md` — matriz de escalonamento quando o agent não consegue prosseguir (low confidence, prompt injection detectado, SLO em risco)

Cobre rigorosamente a **Diretriz 05 — Escopo Restrito** de `lex-agent-construction-directives`.

## Quando Usar

- Após `kata-agent-context-pack-design` (guardrails consomem categorias negativas do context-pack)
- Antes de `kata-dooc-validate` produzir o output final `dooc/{agent}.md` (cross-link no campo Stage 3 de validações)
- Quando há nova categoria de adversarial input identificada (atualização periódica)
...
| `overview_path` | Sim | `docs/{context}/agents/{agent}/overview.md` |
| `system_prompt_path` | Sim | `docs/{context}/agents/{agent}/system-prompt.md` |
| `tools_path` | Sim | `docs/{context}/agents/{agent}/tools.md` |
| `memory_path` | Sim | `docs/{context}/agents/{agent}/memory.md` |
| `context_pack_path` | Sim | `docs/{context}/agents/{agent}/context-pack.md` |
| `--from-pov <path>` | Não | PoV path; herda guardrails experimentados pré-operacional |
...
- `lex-system-prompt` (fonte autoritativa dos 5 controles OWASP críticos)
- `lex-agent-construction-directives::Diretriz 05`
- `context-pack.md::Exemplos negativos` (gabaritos de ataque + comportamento correto)
- `tools.md` (catálogo autoritativo de tools)
- `memory.md` (PII handling por camada)
- `escalation.md` (caminho quando controle dispara)
```

### Passo 2: Redigir `authorization.md`
...
- `lex-auth` — autenticação e autorização das APIs Guardia
- `lex-mcp` — credenciais via env vars
- `guardrails.md::Cross-Tenant Boundary`
- `tools.md::MCP::Auth`
```

### Passo 3: Redigir `escalation.md`
...
> **Agent:** {agent}
> **Owner:** {nome, papel}
> **Canal primário:** {Slack | email | on-call}
...
- `feedback.md::Estados do loop` (estado `escalating`)
- `orchestrator.md::Workflow` (etapa final de escalonamento)
- `codex-incident-response`
- `metrics.md` (alertas que disparam escalonamento)
```

### Passo 4: Validar consistência com context-pack

Para cada categoria negativa em `context-pack.md::Exemplos negativos`:

- Existe controle correspondente em `guardrails.md`
- Existe matriz de escalonamento correspondente em `escalation.md`

Sem cobertura espelhada, registra item de follow-up.

### Validação Final

- [ ] `guardrails.md` cobre os 5 controles OWASP críticos + tool injection + PII + cross-tenant
- [ ] `authorization.md` declara callers permitidos com auth model
- [ ] `escalation.md` declara matriz com gatilho + severidade + SLA + runbook
- [ ] Runbooks placeholders criados em `docs/runbooks/` quando ainda não existem
- [ ] Consistência espelhada com `context-pack.md::Exemplos negativos`
