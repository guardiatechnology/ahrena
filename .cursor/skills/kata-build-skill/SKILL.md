...
description: "Skill build (source → `.build/{slug}/` + zip). Operational implementation of the deterministic build pipeline described in codex-skill-build-pipeline. Reads {paths.skills_root}/{slug}/, validates manifests, builds widgets, freezes scripts, validates tools, and emits {paths.skills_build}/{slug}/ + a testable zip"
---
...
2. When `skip_zip=false`:
   ...
   - Command: `cd .build/{slug} && find . -type f | LC_ALL=C sort | zip -X --no-extra -@ ../{slug}.zip` (or cross-platform equivalent)
