...
This Codex is the reference for **data modeling decisions** in Ahrena projects. Consulted by `warrior-demeter` when designing a new schema, by `warrior-apollo` when it needs to decide table structure, and by `warrior-atlas` when choosing between RDS, Aurora, DynamoDB.

## Context

- **Domain:** data model design — relational, NoSQL, mixed
- **Target audience:** `warrior-demeter`, agents that persist data, reviewers of PRs with migrations
- **Update:** when DDD patterns evolve, new DB services are adopted, or when regulation changes (LGPD, GDPR)
...
- `codex-entities` — base of Ahrena entities
- `codex-migrations-strategy` — migration playbooks
- `warrior-demeter`
