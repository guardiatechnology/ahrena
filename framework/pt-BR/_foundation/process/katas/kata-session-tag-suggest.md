...
Produzir um objeto `tags` válido — `{kind, topics: [...]}` conforme `lex-session-tags` — a partir do primeiro prompt do usuário em uma nova sessão Claude Code, opcionalmente enriquecido pelo front-matter do plano ativo e pelo nome da branch. A kata é pura inferência: retorna as tags sugeridas e NÃO escreve o heartbeat. O chamador (hook do Plan B ou `cry-tags --auto-suggest`) decide se persiste via `kata-session-heartbeat --set-tags`.

## Quando Usar

- Primeiro turno do usuário em uma nova sessão Claude Code quando o heartbeat não tem objeto `tags` e `session_tracking.tags.auto_suggest: true` (acionado pelo hook UserPromptSubmit do Plan B).
- Re-inferência manual disparada pelo usuário via `/cry-tags --auto-suggest`.
- Invocação programática por um warrior que precisa de uma sugestão de tag estruturada antes de escrever.

NÃO usar quando o heartbeat já carrega um objeto `tags` — re-sugestão nesse caso é proibida pela regra 5 de `lex-session-tags`.
...
- `lex-session-tags` — schema, contrato do vocabulário, HARD-GATE
- `kata-session-heartbeat` — kata downstream que escreve o objeto de tag
- `cry-tags` — wrapper para o usuário que invoca esta kata para `--auto-suggest`
- `codex-session-tracking` — seção §9 sobre tags
- `lex-directives` — fonte da verdade de `session_tracking.tags.kinds`
