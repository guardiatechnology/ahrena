...
Este Codex detalha como interpretar e aplicar as diretrizes de tom e estilo de escrita definidas em `naming.tone_and_writing_style` no arquivo `.ahrena/.directives`. Complementa a `lex-tone` (que estabelece a obrigação) com exemplos de texto alinhado e desalinhado e orientações por tipo de conteúdo. Use este Codex ao redigir ou revisar artefatos e comunicação no contexto do Ahrena.

## Contexto

- **Domínio:** Tom, estilo e clareza na produção de texto do framework
- **Público-alvo:** Agentes de IA que redigem artefatos ou comunicação; revisores de conteúdo
- **Atualização:** Quando as diretrizes em `.directives` forem alteradas ou novos exemplos forem necessários
...
| Termo | Definição |
|-------|-----------|
| tone_and_writing_style | Seção opcional em `.ahrena/.directives` (sob `naming`) que lista diretrizes de tom e estilo |
| Estilo direto | Frases objetivas, sem rodeios ou adornos desnecessários |
| Acionável | Conteúdo que leva a uma decisão ou ação clara |

## Referências

- `lex-tone` — Lei que obriga a aplicação do tom e estilo
