...
| `rtk.auto_install_binary` | Placeholder reservado (default `false`). La lógica de auto-install propiamente dicha requiere un ADR posterior — RTK no tiene auto-updater nativo y el canal de instalación varía por entorno (`cargo install`, `brew install`, `curl \| sh`, npm). Cuando es `false` (default), install/update emite una advertencia si `command -v rtk` retorna falso, pero no instala. |
