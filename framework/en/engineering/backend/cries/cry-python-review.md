...
This command invokes the Apollo Warrior (or the agent assuming its role) to perform a systematic code review: verify correctness, type safety, test coverage, security, error handling, and architecture compliance per the backend Lexis and Codex.
...
| Aspect | Cry | Kata |
|--------|-----|------|
| **Nature** | Quick invocation with target and context | Full procedure in 8 steps |
| **Complexity** | Low (1 command) | High (understand, verify, check types/tests/security/architecture, deliver) |
| **Configures agent?** | Yes (assumes Apollo Warrior role) | Yes (defines all review steps) |
| **Example** | "/cry-python-review src/transactions/" | Execute kata-python-review with explicit steps |
...
- `kata-python-review` — Procedure executed by the Apollo Warrior
