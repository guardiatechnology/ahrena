...
Este Codex é a referência para **decisões arquiteturais em mobile**: quando escolher React Native vs. nativo, como estruturar features, gestão de state, persistência offline, sync, navegação, e testes. Consultado por `warrior-iris` ao implementar ou refatorar mobile.

## Contexto

- **Domínio:** aplicações mobile iOS, Android, ou multi-platform.
- **Público-alvo:** `warrior-iris`, agentes que implementam mobile.
- **Atualização:** quando frameworks evoluem (Compose Multiplatform, SwiftUI novos APIs, RN 0.7x), ou padrões de platform emergem.
...
- `lex-frontend-testing`, `lex-frontend-accessibility` — princípios análogos
- `codex-frontend-architecture` — paralelo em web
- `warrior-iris`
