...
This Codex is the operational reference for **behavioral validation** of features at Guardia. Consulted by `warrior-themis` when designing scenarios, by `warrior-athena` when deciding delegation, and by PR reviewers at Gate 3.

BDD here is not a planning methodology nor a test framework — it is a **black-box validation mechanism** that protects against "we built the wrong thing", complementing the AC↔test traceability already required by `lex-issue-driven`.

## Context

- **Domain:** post-implementation behavioral validation (Phase 8 of the Issue-Driven flow).
- **Audience:** `warrior-themis`, `warrior-athena`, `warrior-hera`, reviewers opening or approving PRs.
- **Update trigger:** when the scenario taxonomy proves insufficient, when new source types are adopted (e.g., Figma specs), or when anti-patterns repeat in reviews.
...
- `lex-bdd-gherkin-format` — mandatory Gherkin format
- `lex-bdd-no-framework-coupling` — test implementation without a step-runner
- `codex-gherkin` — adopted Gherkin manual
- `codex-test-strategy` — test level selection
- `kata-bdd-scenarios-design` — production of `07-bdd-scenarios.md`
- `kata-bdd-validate-implementation` — production of `08-bdd-validation-report.md`
- `warrior-themis` — Phase 8 agent
- `lex-issue-driven` — flow preceding Phase 8
