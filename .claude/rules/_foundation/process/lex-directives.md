...
| `pr_cost_tracking.enabled` | When `true`, enable the token cost stamp (Claude Code) on PR bodies via `kata-pr-cost-stamp`. Default `false`. The only key consumed in this iteration. The subkeys `currency`, `include_cache_breakdown`, `window_override_days`, `mask_absolute_cost` are declared in `.directives.sample` as reserved for future iterations. See `codex-pr-cost-tracking`. |
