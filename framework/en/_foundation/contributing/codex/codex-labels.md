...
This Codex documents all labels used in Guardia repositories and the GitHub Issue Types configured for the organization. It serves as the single source of truth for: which labels apply to which artifact type, which are applied automatically vs. manually, and how PR size is calculated. It is consulted by `kata-contributing-issue`, `kata-contributing-pr`, and `lex-issue-quality`.

## Context

- **Domain:** Contributing workflow — label governance
- **Target audience:** AI agents, developers, and community contributors
- **Canonical source:** `.github/labeling/labels.yml` in `guardiatechnology/project-automations-experiments`
- **Update:** When labels are added, removed, or redefined in `labels.yml`
...
| `documentation 📃` | `tech-task` | Documentation improvements or additions |
| `ci 🏗️` | `tech-task` | CI/CD or pipeline changes |
| `enhancement 🔝` | `tech-task` | Enhancement to an existing feature |
| `evolvability ♻️` | `tech-task` | Refactoring, clean code, maintenance |
...
| Term | Definition |
|------|------------|
| Issue Type | GitHub organization-level classification: Task, Bug, Feature |
| Size label | Auto-applied label reflecting the PR diff size (ignoring generated files) |
| Tool-assigned label | Auto-applied label indicating which AI tool or bot created the PR |
| Label mirroring | Applying the same labels from an issue to its corresponding PR |
