...
- `codex-session-tracking` — §9 tags schema and r/w contract
- `kata-session-heartbeat` — atomic merge of `tags` into the heartbeat JSON
- `kata-session-tag-suggest` — inference procedure on the first user turn
- `cry-tags` — user override commands
- `lex-directives` — `session_tracking.tags.*` keys
- `lex-no-plans-under-docs` — sibling rule keeping tags out of `docs/` (heartbeat-only)
