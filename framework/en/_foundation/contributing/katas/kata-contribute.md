...
This Kata defines the standardized procedure for opening a Pull Request in the project's origin repository using GitKraken MCP tools. It ensures that every contribution follows the unified flow defined in `codex-contributing`.

## When to Use

- When changes are ready for submission to the repository
- When the user requests to create a PR
- When invoked by `cry-contribute pr`
...
| `repository_organization` | Extracted from remote (e.g., `guardiafinance`) |
