...
Este manual define os padrões de arquitetura para aplicações Python backend. O objetivo é garantir estrutura consistente entre serviços, separação clara de responsabilidades, testabilidade em cada camada e um codebase que escala com o time sem acumular complexidade acidental.

A arquitetura segue os princípios de **Clean Architecture** (Hexagonal / Ports & Adapters): a lógica de domínio é independente de frameworks, bancos de dados e mecanismos de transporte. As dependências apontam para dentro — infraestrutura depende do domínio, nunca o contrário.

## Context

- **Domain:** aplicações Python backend usando FastAPI, SQLAlchemy, Pydantic.
- **Target audience:** implementadores, arquitetos e agentes de IA que constroem ou mantêm serviços Python.
- **Update trigger:** quando os padrões arquiteturais evoluem ou novos padrões de infraestrutura são adotados.
...
| Termo | Definição |
|-------|-----------|
| Port | Interface abstrata (Protocol) que define um contrato entre camadas |
| Adapter | Implementação concreta de um port (ex.: repositório SQLAlchemy) |
| Use Case | Orquestração de operações de domínio no nível de aplicação |
| Composition Root | Onde as implementações concretas são conectadas aos ports (DI) |
