...
```
Progreso:
- [ ] 1. Validar inputs y resolver paths
...
- [ ] 6. Escribir overview.md y validar
```

### Paso 1: Validar inputs y resolver paths

1. Se confirma que `--context`, `--problem`, `--value-metric` y `--kind` están completados. Sin cualquiera de ellos, aborta con mensaje claro.
2. Se resuelve `docs/{context}/agents-pov/` a partir del `{context}` informado. Si el directorio ya existe y contiene `overview.md`, se alerta al usuario y se exige confirmación (`--force`) — sobreescribir un PoV existente es decisión consciente.
...
3. El resultado es la primera sección de `overview.md`.
...
3. Se señala explícitamente en `overview.md` cada caso de uso que **no** se aborda en este PoV (referencias futuras van a otro PoV o a Mêtis).
...
3. El resultado se convierte en la sección "Criterio de descontinuación" de `overview.md`.

### Paso 6: Escribir overview.md y validar

1. Se genera `docs/{context}/agents-pov/overview.md` con secciones: Problema del cliente, Caso de uso primario, Fuera de alcance, Persona, Stage, Value metric leading (copia literal de `--problem` y `--value-metric`), Criterio de descontinuación, Próximos pasos.
2. Próximos pasos siempre listan los 6 katas siguientes a ejecutar: `kata-pov-system-prompt`, `kata-pov-tools-select`, `kata-pov-context-curate`, `kata-pov-observability-instrument`, `kata-pov-feedback-attach`, `kata-pov-value-track`.
3. Se aplica `kata-artifact-self-review` al archivo generado antes de entregar.

### Validación Final

- [ ] `overview.md` existe en `docs/{context}/agents-pov/`
- [ ] Contiene literalmente `stage: pre-operational` en el bloque persona
- [ ] Caso de uso primario es exactamente 1
- [ ] Criterio de descontinuación tiene ventana + métrica + threshold
- [ ] Próximos pasos listan los 6 katas POV restantes
...
**Modelo:** Este Kata aplica la Directriz 05 (`lex-agent-construction-directives`) al ciclo PoV. Foco en estrecho + criterio de salida evita zombis. Consumido por `warrior-claudionor` como primer paso de `cry-pov`.
