...
This Codex is the reference for using **bash** and **PowerShell** in the Ahrena context. It defines how to declare the terminal type in directives, when to use each, common equivalences between them, and good practices for documentation and scripts. It is used together with `lex-terminal-type`.

## Context

- **Domain:** Execution of shell commands; scripts and examples in documentation.
- **Target audience:** AI agents that execute or generate terminal commands; developers who maintain scripts and documentation.
- **Update:** When the project adopts another shell or terminal conventions change.
...
| Term      | Definition |
|-----------|------------|
| bash      | Default shell on Linux and macOS; available on Windows via WSL or Git Bash. |
| PowerShell | Microsoft shell; available on Windows (Windows PowerShell or PowerShell Core) and on Linux/macOS (PowerShell Core). |
| terminal  | Type of command interpreter (bash or PowerShell) used in the project to execute and document shell commands. |

## References

- `lex-terminal-type` — Law that requires using the terminal type defined in directives.
