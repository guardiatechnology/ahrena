...
- **Operates the `to review ↔ review` sub-cycle** per `lex-agent-planning` Table A (Axis A — dev cycle):
  - **Entry:** upon receiving a review trigger (via `cry-review-pr` or post-Athena invocation), invokes `kata-load-plan-from-subissue` to materialize `.claude/plans/plan-{N}.md` from the canonical Issue body. Confirms that the PR is in `status: to review` and moves it to `status: review` (label on PR + Issue per `lex-issue-status` intra-artifact mutex)
...
2. **Phase 0 — Collection:**
   ...
   - Reads local `.ahrena/issues/{N}/*` when present and the referenced `.claude/plans/plan-{N}.md` cache (per  — canonical plan body lives in the Issue)
