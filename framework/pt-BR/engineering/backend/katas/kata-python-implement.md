...
Esta Kata define o procedimento para implementar um feature em uma aplicação Python backend: entender o requisito, identificar camadas afetadas, projetar interfaces, implementar lógica de domínio com testes, construir adaptadores de infraestrutura, validar com a cadeia de qualidade completa e entregar.

## When to Use

- Quando um novo feature requer implementação em um serviço Python backend
- Quando invocado por `cry-python-implement` ou pelo Warrior Apollo
- Quando se adiciona um novo endpoint, caso de uso, repositório ou entidade de domínio

## Inputs

| Input | Obrigatório | Descrição |
|-------|:-----------:|-----------|
| Descrição do feature | Sim | Descrição textual do feature, comportamento esperado e critérios de aceitação |
| Contexto ou escopo | Não | Restrições (ex.: padrões existentes a seguir, requisitos de performance, spec OAS relacionada) |
| Camadas afetadas | Não | Quais camadas estão envolvidas (domínio, infraestrutura, HTTP). Se omitido, o agente as identifica |
