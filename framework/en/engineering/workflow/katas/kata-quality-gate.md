...
|-------|:--------:|-------------|
| Phase 2 requirements | Yes | `.issues/{n}/02-requirements.md` (numbered ACs) |
| Phase 3 architecture | Yes | `.issues/{n}/03-architecture.md` (component table — scope) |
| Phase 4 implementation | Yes | Code + tests in the working tree |
| Phase 5 review | Yes | `.issues/{n}/05-security-review.md` (must be `approved`) |
...
- [ ] 10. Persist to .issues/{n}/06-quality-report.md
...
3. **Legitimate exceptions** (do not flag):
   ...
   - Documentation generated by the flow itself (e.g., `.issues/{n}/*`).
...
### Step 10: Persist to `.issues/{n}/06-quality-report.md`
...
|--------|--------|-------------|
| Gate report | Markdown with 6 checks + traceability matrix | `.issues/{n}/06-quality-report.md` |
...
- **Fixed destination:** `.issues/{n}/06-quality-report.md` (per `lex-issue-driven`). In per-layer mode, the report accumulates one section per layer plus a final aggregate section.
