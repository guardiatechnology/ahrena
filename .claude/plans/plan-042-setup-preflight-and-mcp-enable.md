...
- [x] **Step 15 — Review round 2 (Argos).** **Done 2026-05-10**: 2 BLOCKER + 4 WARNING acionáveis endereçados. (a) BLOCKER #1: `_ensure_server_in_directives` tinha o mesmo bug EOF-no-newline do path REMOVE — `_ACTIVE_BLOCK_RE` agora usa `(?:\n|$)` e a função normaliza body para terminar em `\n` antes de inserir, evitando que o servidor novo caia no meio da lista. (b) BLOCKER #2 já resolvido em `2d1fd4b` (gemini's `(?:\n|$)`). (c) WARNING segurança: `install_tool` em `preflight.py` migrou de `subprocess.run(cmd, shell=True)` para `shlex.split(cmd) + shell=False` — fecha a superfície de injection caso futuro consumidor componha `ToolSpec` a partir de input não-estático; contrato documentado no docstring. (d) WARNING UX: política de `requires` desconhecido refinada — fatal só em `--non-interactive` (CI nunca passa por dep desconhecida); em interactive, prompt `Proceed anyway? [y/N]` com default no. (e) WARNING testabilidade: `tools/ahrena-mcp/tests/test_preflight.py` e `test_mcp_enable.py` adicionados — 30 novos testes cobrindo regex BLOCKERs, `_python_executable_name` em Windows/Linux/Darwin, `_parse_semver` edge cases, `detect_os` branches, `check_tool` (presente/ausente/versão antiga), `install_tool` (sem hint, dry-run, shell=False), `_REQUIRES_RESOLVER` (conhecido/desconhecido), `_remove_from_platform_config` (presente/ausente/arquivos faltando). Refator menor: `_python_executable_name()` extraído como helper para permitir teste com monkeypatch. Pendente para follow-up issues: (i) backfill de traduções ES em `codex-mcp-{github,notion,figma}` (gap pré-existente), (ii) execução de bootstrap VM-clean.
- [x] **Step 16 — Review round 2 closeout (ES backfill + PR body).** **Done 2026-05-10**: dois itens "Still open" do Argos endereçados in-PR (não como follow-up): (a) **PR body** ganhou bullet sob Summary citando `codex-ahrena-mcp` e a entry `ahrena` no `.cursor/mcp.json` como adições documentais que vieram pelo sync do `install.py` (origem PR #76); (b) **ES backfill completo** em `framework/es/_foundation/tooling/codex/codex-mcp-{github,notion,figma}.md` — 7 seções traduzidas do EN: github ganhou `### Ejemplo de uso: crear PR con body estructurado`; notion ganhou `### Parámetros de las herramientas más usadas` + `### Ejemplo de uso: crear página de documentación`; figma ganhou `### Cómo obtener el File ID de Figma` + `### Parámetros de las herramientas más usadas` + `### Mapeo de variables a design tokens` + `### Ejemplo de uso: extraer color tokens`. Verificação: contagem de h3 agora idêntica nas 3 línguas para os 3 codexes (6/6/6 em github e notion; 8/8/8 em figma). Tradução seguiu `lex-language` (estrutura preservada) + `lex-language-es` (Spanish neutro, voz impessoal). Issue #83 fechada.
