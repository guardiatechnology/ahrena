...
Executar revisão de arquitetura AWS (documento de design ou diff de IaC), verificando aderência às Lexis aplicáveis (`lex-aws-security`, `lex-aws-iac`, `lex-aws-cost`), aos 6 pilares do Well-Architected e às práticas do catálogo de serviços. Produz relatório estruturado com achados categorizados por severidade, aplicável em PR review ou como revisão periódica de infraestrutura existente.

## Quando Usar

- Revisão de PR de IaC antes do merge
- Revisão de documento de arquitetura (antes do Gate 1 no fluxo Issue-Driven)
- Auditoria periódica de conta/workload existente
- Preparação para AWS Well-Architected Review formal

## Inputs

| Input | Obrigatório | Descrição |
|-------|:-----------:|-----------|
| Alvo | Sim | PR de IaC (`git diff`) ou documento `03-architecture.md` ou conta/workload existente |
| Escopo | Sim | Componentes a revisar — full review ou foco em um pilar |
| Contexto | Não | Requisitos originais, SLAs, compliance aplicável |
...
- `codex-aws-well-architected` — 6 pilares detalhados
- `codex-aws-services` — catálogo para comparar escolhas
- `kata-quality-gate` — integra ao fluxo Issue-Driven
