...
Stacked Pull Requests are a chain of PRs where each branch targets the previous one, allowing a large feature to be split into reviewable layers in isolation. Instead of a single 2k-line PR that gets rubber-stamped, the work is N PRs of 200-400 lines each, chained together, merged from bottom to top.

This Codex is the canonical manual for deciding **when** to stack and **how** to operate the chain. The operation described here uses only `git` + `gh` CLI (the **vanilla** path), works on any GitHub repository. Tool-based paths (git-spice, gh-stack) reuse the concepts defined here and add automation on top.

Adoption is **opt-in** and governed by the `stacked_prs.tool` directive in `.ahrena/.directives` (default `vanilla`).

## Context

- **Domain:** decomposition of large features into stacked PRs
- **Audience:** all contributors (humans and agents); Athena in the Issue-Driven flow (future)
- **Update:** when referenced Lexis change, when new tool paths land (plan-005, plan-006), or when the Decision Checklist is refined based on real usage
...
- `lex-issue-first` — issue mandatory before the branch
- `lex-issue-quality` — 5 canonical issue criteria
- `lex-git-branches` — branch naming pattern
- `lex-git-worktrees` — exception declared for stacks
- `lex-pr-quality` — HARD-GATE applied per PR in the stack
- `lex-protected-trunk` — trunk protection preserved
- `lex-conventional-commits`, `lex-small-commits`, `lex-signed-commits` — commit disciplines preserved
- `kata-stacked-pr-create` — Pre-flight (Phase 0) + chain creation
- `kata-stacked-pr-rebase` — manual cascade rebase
- `kata-stacked-pr-merge` — bottom-up merge
- `cry-new-stacked-pr` — shortcut that invokes kata-create
- `kata-contributing-pr` — single-PR flow (fallback when the Decision Checklist fails)
