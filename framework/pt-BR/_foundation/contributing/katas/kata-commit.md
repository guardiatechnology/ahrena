...
| `--warrior <nome>` | Não | Nome do warrior que invoca o kata (ex.: `apollo`, `athena`, `hephaestus`). Habilita o roteamento default-warrior quando `warriors_default_author.enabled=true` e o nome está em `warriors_default_author.apply_to`. Quando omitido, o fluxo é sempre commit local (humano). |
...
### Passo 4: Resolução do Autor (humano ou identidade default dos warriors)

Antes de chamar `git commit`, o kata decide entre dois caminhos de autoria:

1. **Carregar `scripts/ahrena-auth.sh`** (sempre — é no-op quando `warriors_default_author.enabled=false`):
   ```bash
   source scripts/ahrena-auth.sh
   ```
   Quando o diretivo está desligado, o script retorna imediatamente sem exportar nada. Quando ligado, ele exporta `GH_TOKEN_AHRENA_WARRIORS_DEFAULT` (token de instalação do GitHub App) e as variáveis `GIT_AUTHOR_*` / `GIT_COMMITTER_*` apontando para a identidade default da frota (`ahrena-bot[bot]`).

2. **Ler `warriors_default_author.enabled` e `warriors_default_author.apply_to`** em `.ahrena/.directives`.

3. **Decisão de roteamento:**
   - Se `warriors_default_author.enabled == true` E o input `--warrior <nome>` foi fornecido E `<nome>` está em `warriors_default_author.apply_to`: usar o **caminho default-warrior** (Passo 5a abaixo).
   - Caso contrário (ausência de `--warrior`, master switch desligado ou warrior fora do `apply_to`): usar o **caminho humano** (Passo 5b).

4. **Caminho default-warrior** — montar o `Co-authored-by` do humano que dirige a sessão:
   ```bash
...
#### Passo 5a: Caminho default-warrior (servidor)

Quando o roteamento do Passo 4 selecionou o caminho default-warrior:
...
- [ ] **Caminho default-warrior (5a):** `git log -1 --format='%an <%ae>'` mostra `ahrena-bot[bot]` e o commit aparece com o badge **Verified** no GitHub
- [ ] **Caminho default-warrior (5a):** o body do commit contém o trailer `Co-authored-by: <humano>` quando `warriors_default_author.commit_co_author=human`
...
- No caminho default-warrior (Passo 5a), a assinatura é servida pelo token de instalação do GitHub App; não é necessário GPG local
...
- `lex-signed-commits` — Assinatura GPG obrigatória (caminho humano) ou assinatura via App (caminho default-warrior)
...
- `codex-git-workflow` — Seção "Identidade de autor" descreve o roteamento humano vs identidade default dos warriors
- `scripts/ahrena-auth.sh` — Gate `warriors_default_author.enabled` + resolução de credenciais do GitHub App
- `scripts/ahrena-api-commit.sh` — Commit via Git Data API (caminho default-warrior)
