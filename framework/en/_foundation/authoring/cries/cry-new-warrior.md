...
Quick command to create a new Warrior in Ahrena. Invokes `kata-create-warrior`, which consults `codex-warriors` and the official template to produce a complete specialized agent in all three mandatory languages.
...
| Aspect | Cry | Kata |
|--------|-----|------|
| **Nature** | Quick invocation (1 command) | Complete procedure (7 steps) |
| **Complexity** | Low (role + domain) | High (identity, responsibilities, consultation) |
| **Configures agent?** | No | Yes (defines behavior) |
| **Example** | `/cry-new-warrior "Architect"` | 7-step workflow with checklist |

## References

- `kata-create-warrior` — Procedure executed by this Cry (the Kata consults the applicable quality criteria; see Kata documentation)
- `templates/warrior-sample.md` — Base template
