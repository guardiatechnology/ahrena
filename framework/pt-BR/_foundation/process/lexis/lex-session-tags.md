...
- `codex-session-tracking` — §9 schema das tags e contrato de leitura/escrita
- `kata-session-heartbeat` — merge atômico de `tags` no heartbeat JSON
- `kata-session-tag-suggest` — procedimento de inferência no primeiro turno do usuário
- `cry-tags` — comandos de override do usuário
- `lex-directives` — chaves `session_tracking.tags.*`
- `lex-no-plans-under-docs` — regra irmã que mantém as tags fora de `docs/` (apenas no heartbeat)
