...
Este Codex cataloga os erros padronizados utilizados nas APIs e eventos da plataforma Guardia. Cada erro é identificado por seu `code` (UPPER_SNAKE_CASE com prefixo `ERR{HTTP}_`) e detalhado pelos `reason` possíveis, com mensagem orientada ao desenvolvedor, elegibilidade de retry e tratamento sugerido.

> **Importante:** Todo erro DEVE seguir a estrutura definida em [codex-error-handling](codex-error-handling.md). Novos `reason` DEVEM ser justificados e adicionados a este catálogo.

## Contexto

- **Domínio:** catálogo de erros emitidos por endpoints HTTP e consumidores/processadores de eventos.
- **Público-alvo:** implementadores de APIs/eventos, integradores e clientes que tratam respostas de erro.
- **Atualização:** sempre que um novo `reason` for cunhado em uma especificação do Hub ou criado em um endpoint específico.
...
- [codex-error-handling](codex-error-handling.md) — estrutura padronizada de erros
- [codex-idempotency](codex-idempotency.md) — `Idempotency-Key` e `Content-Digest`
- [codex-restful-headers](codex-restful-headers.md) — headers padrão e custom
- [codex-restful-pagination](codex-restful-pagination.md) — erros de paginação
- [codex-restful-status-codes](codex-restful-status-codes.md) — uso correto de HTTP status
