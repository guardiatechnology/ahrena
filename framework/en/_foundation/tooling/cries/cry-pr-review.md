...
Shortcut that invokes `kata-pr-review`. The Kata guides the `purpose=review` marking (via the `GUARDIA_PURPOSE` env var or the prompt heuristic) and triggers the review. Without that mark, review turns end up in the `dev` bucket and pollute the reading of the effort that produced the PR.
...
- `kata-pr-review` — Detailed marking + trigger procedure
- `codex-pr-cost-tracking` — Manual with the `purpose` cascade and the `Review` block layout
- `kata-pr-cost-stamp` — Stamps the block with the review count on the PR
