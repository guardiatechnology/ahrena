...
Shortcut that invokes `warrior-phanes` to promote approved insights into an Idea under `docs/discovery/{topic}/ideas/`. The Cry **does not** invoke Lexis or Codex directly — it only triggers the Warrior, which internally executes `kata-ideation-from-insight`, validates HARD-GATE 1 of `lex-discovery-flow`, and consults `codex-discovery-artifacts`.
...
- `warrior-phanes` — agent invoked by this Cry
- `kata-ideation-from-insight` — procedure executed internally
- `lex-discovery-flow` — applicable law (consulted by the warrior, not by the cry)
- `codex-discovery-artifacts` — schema of insights and Ideas (consulted by the warrior)
- `cry-discovery` — complementary Cry (upstream insight production)
