...
A partir de requisitos funcionais e não-funcionais (tráfego esperado, SLAs, compliance, orçamento), projetar uma arquitetura AWS completa: escolha de serviços justificada pelos 6 pilares do Well-Architected, diagrama, lista de recursos, estimativa de custo mensal, análise de riscos, e IaC inicial (scaffolding) em Terraform ou CDK.

## Quando Usar

- Nova feature/sistema que exige infraestrutura AWS
- Redesign de workload existente (migração de arquitetura, escalar, reduzir custo)
- Quando `warrior-athena` delega a Atlas na Fase 3 do fluxo Issue-Driven para definir arquitetura cloud

## Inputs

| Input | Obrigatório | Descrição |
|-------|:-----------:|-----------|
| Descrição funcional | Sim | O que o sistema faz, fluxos principais |
| Requisitos não-funcionais | Sim | Tráfego (req/s, tamanho payload), SLA, RTO/RPO, compliance, orçamento estimado |
| Stack existente | Sim | Contas AWS, VPCs, serviços já em uso, IaC tool do projeto |
| Restrições | Não | Região obrigatória, serviços proibidos, integrações externas |
...
Estrutura (quando no fluxo Issue-Driven, este documento **é parte** de `docs/issues/issue-{n}/03-architecture.md` ou referenciado dele):
...
|-------|---------|---------|
| Documento de arquitetura | Markdown | `docs/issues/issue-{n}/03-architecture.md` (ou arquivo dedicado) |
...
- `codex-aws-services` — catálogo de serviços
- `kata-adr-write` — para decisões arquiteturais relevantes
- `codex-issue-workflow` — integração no fluxo Issue-Driven
