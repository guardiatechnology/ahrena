...
Produce `docs/issues/issue-{n}/07-bdd-scenarios.md` deriving Gherkin scenarios **exclusively** from specification sources (GitHub Issue, linked Notion pages, and Phase 1-3 flow artifacts). The artifact is black-box validation of the expected behavior and serves as the contract for the next stage (`kata-bdd-validate-implementation`).

## When to Use

- Phase 8.1 of the flow orchestrated by `warrior-athena`, **after** Gate 2 passes.
- Whenever a feature or bugfix needs an updated BDD scenario set (e.g., ACs amended in a new Gate 1 iteration).
- On demand when the team requests an independent BDD validation on an already-implemented feature.
...
| `repo` | Yes | `owner/repo` (e.g., `guardiafinance/ahrena`) |
| Flow artifacts | Yes | `01-brief.md`, `02-requirements.md`, `03-architecture.md` in `docs/issues/issue-{n}/` |
| Notion pages | No | Referenced by the Issue or by the artifacts; gathered via `kata-mcp-notion-read` |
| Linked ADRs | No | In `docs/adr/` when referenced by the architecture |
...
- `lex-bdd-gherkin-format` — declarative format
- `lex-bdd-no-framework-coupling` — no step-runner
- `codex-bdd` — principles and taxonomy
- `codex-gherkin` — syntax and patterns
- `lex-issue-driven` — Issue-Driven flow (Phase 8)
- `kata-bdd-validate-implementation` — next stage (Phase 8.2)
- `kata-mcp-github-read`, `kata-mcp-notion-read` — read via MCP
- `warrior-themis` — agent that invokes this Kata
