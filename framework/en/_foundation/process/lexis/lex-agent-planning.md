...
This revision changes the plan **storage medium**: the canonical content lives in the GitHub **Issue body**; `.plans/{N}.md` is the AI's local cache (gitignored); `.ahrena/issues/{N}/` holds the Phase artifacts of the Issue-Driven flow (committed). A dedicated plan file in `.claude/plans/*.md` is no longer the canonical source.
