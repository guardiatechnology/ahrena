...
This Kata defines the step-by-step procedure to configure GPG commit signing on a developer's machine so that every commit and tag is marked as **Verified** on GitHub, satisfying `lex-signed-commits`.

## When to Use

- When setting up a new development machine for the first time
- When a developer has not yet configured GPG signing
- When invoked by a Warrior or as part of a contributor onboarding flow
- When commits show "Unverified" on GitHub

## Inputs

| Input | Required | Description |
|-------|:--------:|-------------|
| GitHub account email | Yes | Must match the email in the GitHub account and in the GPG key |
| Key passphrase | No | Chosen during generation; strongly recommended |
...
- `lex-signed-commits` — Law requiring GPG-signed commits in Guardia repositories
- `codex-git-workflow` — Complete contribution workflow
