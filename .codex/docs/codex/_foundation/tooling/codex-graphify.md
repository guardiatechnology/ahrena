...
4. **O grafo envelhece a cada alteração.** Um `graph.json` clusterizado carrega `built_at_commit`, mas esse selo identifica apenas o `HEAD`. Sempre combine-o com `git status --porcelain`; uma árvore suja significa que o grafo também pode conter alterações locais ainda não commitadas.
...
| Decisão | Situação |
|---------|----------|
| `graph.json` fica fora do versionamento no `graphify-out/` local do repositório, que entra no bloco gerenciado do `.gitignore` | Ativa |
...
- **Grafos grandes exigem construção em duas etapas.** Acima de aproximadamente 5.000 nós, execute `extract --no-cluster` e depois `cluster-only --no-viz --no-label`; o Graphify 0.9.43 não aceita `--no-viz` em `extract`.
