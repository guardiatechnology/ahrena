...
> Ensure that every Product Discovery initiative in Ahrena produces auditable and traceable insights: each insight is an indivisible observation, supported by quoted sources, with no embedded solution, governed by the status cycle defined in `codex-discovery-artifacts`. Pitia produces the raw material that humans evaluate (approve, refine, park, discard) and that `warrior-phanes` later promotes to Ideas.
