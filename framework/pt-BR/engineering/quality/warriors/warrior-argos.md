...
- **Opera o sub-ciclo `to review ↔ review`** per `lex-agent-planning` Tabela A (Eixo A — dev cycle):
  - **Entrada:** ao receber trigger de revisão (via `cry-review-pr` ou invocação pós-Athena), invoca `kata-load-plan-from-subissue` para materializar `.claude/plans/plan-{N}.md` a partir do body canônico da Issue. Confirma que o PR está em `status: to review` e move para `status: review` (label + Issue per `lex-issue-status` mutex intra-artefato)
...
2. **Fase 0 — Coleta:**
   ...
   - Lê `.ahrena/issues/{N}/*` local quando presente e o cache `.claude/plans/plan-{N}.md` referenciado (per  — corpo canônico do plano vive na Issue)
