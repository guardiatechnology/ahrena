...
This Codex documents the **`ahrena` MCP server** — the framework's internal server that exposes Lexis, Codex, Katas, Warriors, and Cries as read-only queryable tools to any MCP client. Unlike `github`/`notion`/`figma` (external opt-in integrations), `ahrena` is **default-on**: it activates in every project that adopts Ahrena. Agents consult it when they need surgical access to the framework (without importing whole files) and external agents use it when they cannot access `.claude/`/`.cursor/` mirrors (Strands, automations, scripts).

## Context

- **Domain:** Read-only consultation of the Ahrena framework (Lexis, Codex, Katas, Warriors, Cries artifacts) and reading of `.ahrena/.directives`.
- **Audience:** AI agents in Cursor and Claude Code; external agents (Strands, `apollo-agents`); scripts that read the framework programmatically.
- **Update:** When new tools are added to the server (mutation in the tools contract); when the distribution channel changes (release v1 GitHub Release → v2 PyPI).
...
- `lex-mcp` — Lexis governing the use of MCP servers.
- `codex-mcp-common` — Shared MCP patterns (authentication via env vars, fallback).
- `framework/mcp/ahrena.json` — Canonical configuration merged by the installer.
- `tools/ahrena-mcp/` — Server source code (in the Ahrena repo).
- `tools/ahrena-mcp/README.md` — Technical manual for the package (installation, smoke test).
- `tools/ahrena-mcp/CHANGELOG.md` — Server change history.
- `.claude/plans/plan-021-ahrena-mcp-server.md` — Full implementation plan, including §Release & Distribution.
