...
- After `kata-pov-tools-select` and `kata-pov-observability-instrument` (value-metrics referenced)
- When the PoV tier is defined (default: tier-3/4)
- When a round of operation reveals that the declared feedback is not being captured (re-execution)
