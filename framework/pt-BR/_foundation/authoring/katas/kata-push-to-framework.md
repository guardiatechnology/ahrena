...
Este Kata define o procedimento para incorporar ao framework canônico os artefatos que foram criados no espaço do projeto (`.ahrena/artifacts/`). Suporta dois modos: **local** (cópia para `framework/` no repo atual, com i18n) e **remoto** (sincronização com o repositório do framework no GitHub via MCP do GitHub). Opcionalmente remove as cópias do projeto após a incorporação.
...
|-------|:-----------:|-----------|
| Modo | Sim* | `local` ou `remote`. Local: destino = `paths.framework` no repo atual. Remote: destino = repositório do framework no GitHub (PR/push via MCP do GitHub). *Default pode ser `local` se `paths.framework` existir no repo. |
...
| Remover do projeto | Não | Se "sim", remove os arquivos de `.ahrena/artifacts/` após copiar para o framework (local) ou após envio bem-sucedido (remote). Default: "não" |
