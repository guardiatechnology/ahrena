...
This Codex is the canonical manual for agent task planning (Issue-as-plan three-layer model). It complements `lex-agent-planning` (the Law) with templates, fill-in examples, load/flush cadence, transition owners, and guidelines for edge cases. Every agent that creates or maintains plans MUST consult this Codex.
...
> **Legacy model (deprecated):** files `plan-{NNN}-{slug}.md` in `.claude/plans/` were migrated to `.ahrena/issues/_legacy/` in the  PR. Do not create new files in this format — the Issue body is now canonical; `.plans/{N}.md` is the local cache named by the Issue number.
...
| Field | Rule |
|---|---|
| `{N}` | Number of the corresponding GitHub Issue. No padding, no prefix — `.plans/96.md`, not `.plans/.md` or `.plans/96-slug.md` |
...
> **Legacy front-matter:** plans in `.ahrena/issues/_legacy/` (deprecated) keep historical YAML front-matter (`plan_id`, `status`, `claude_session`, `merge_commit`, `closed_at`). This format is recognized for audit, but do NOT replicate it in new plans.
...
**Canonical state:** the `status:` lives as a **label** on the GitHub Issue (and on the PR, starting at `to review`). There is no more "plan front-matter" — the Issue body is canonical; `.plans/{N}.md` is the regenerable cache. Legacy plans in `.ahrena/issues/_legacy/` keep historical front-matter for audit, without retrofit.
...
1. Issue body via `kata-flush-plan-to-issue` (canonical — only if the local cache was edited).
...
## 9. Load/flush cadence

Synchronization between the Issue body (canonical) and the local cache `.plans/{N}.md` happens at **3 canonical triggers** (per  Open Question #3):
...
6. **Canonical plan lives on GitHub.** Do not create `.claude/plans/*.md` files as canonical (legacy pre- model). The Issue body is canonical; `.plans/{N}.md` is the regenerable cache; `.ahrena/issues/{N}/` carries the Phase artifacts.
