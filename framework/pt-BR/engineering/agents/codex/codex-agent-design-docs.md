...
Este Codex é o manual canônico dos documentos de design de agent da plataforma Guardia. Define a estrutura física de `docs/{context}/agents/{agent}/` (13 arquivos Hub & Spoke), o snapshot de governança em `docs/{context}/dooc/{agent}.md`, e a correlação m:n com features via `feature-agent-map.md`. A Lei correspondente está em `lex-agent-design-docs`.

Os 13 arquivos por agente mapeiam às **6 Diretrizes de Construção de Agentes** detalhadas em `codex-agent-construction-directives` (Notion é a fonte viva). Este Codex padroniza a forma física dos documentos; `codex-agent-construction-directives` é a fundação conceitual (Piaget, stage tags, rigor diferencial por estágio, formato de evidências DoOC).

## Contexto

- **Domínio:** organização de artefatos de design de agent na plataforma Guardia
- **Público-alvo:** `warrior-metis` (autora), `warrior-apollo-agents` (consumidor de implementação), `warrior-athena` (Gate 2 quando feature toca `docs/**/agents/**`), autores humanos, revisores de PR
- **Princípio paralelo:** Feature e Agent são eixos paralelos sob a Capability (1..n ↔ 1..n), não hierárquicos. Este Codex governa o eixo Agent. O eixo Feature vive sob `codex-feature-design-docs`. A correlação é declarada em `feature-agent-map.md` (seção 15) com cross-refs bidirecionais.
- **Atualização:** a cada mudança na estrutura ou templates (ADR obrigatório quando muda categoria reservada, item da DoOC ou stage)
...
- `docs/{context}/dooc/{agent}.md` — snapshot da DoOC
- `docs/{context}/feature-agent-map.md` — correlação m:n com features
- `lex-agent-design-docs` — Lei correspondente (5 preconditions HARD-GATE)
...
### 2. `agents/{agent}/orchestrator.md`
...
### 3. `agents/{agent}/specialists/{name}.md`
...
### 6. `agents/{agent}/reasoning-loop.md`
...
### 8. `agents/{agent}/context-pack.md`
...
### 9. `agents/{agent}/system-prompt.md`
...
### 12. `agents/{agent}/authorization.md`
...
### 15. `feature-agent-map.md` (no root da capability)
...
> Este template é **idêntico** ao da seção 6 de `codex-feature-design-docs` — cada eixo (Feature e Agent) é autocontido por construção. Atualização do template é coordenada entre os dois Codex via ADR.
...
- `lex-agent-design-docs` — Lei correspondente (HARD-GATE com 5 preconditions)
- `lex-agent-construction-directives`, `codex-agent-construction-directives` — 6 Diretrizes + DoOC (consumidos pelos templates de `agents/{agent}/` e `dooc/`)
- `codex-feature-design-docs`, `lex-feature-design-docs` — eixo paralelo Feature Design
...
- `warrior-metis` — agente principal (autora dos documentos do eixo Agent)
- `warrior-apollo-agents` — consumidor canônico de `docs/{context}/agents/` durante implementação
- `warrior-athena` — Gate 2 quando feature toca `docs/**/agents/**`
- `warrior-prometheus` — coordena a correlação bidirecional Feature ↔ Agent
