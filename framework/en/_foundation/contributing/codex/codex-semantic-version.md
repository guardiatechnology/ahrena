...
This Codex is the reference for applying Semantic Versioning 2.0 (SemVer) in Guardia repositories. It defines when to increment MAJOR, MINOR, or PATCH, how SemVer relates to Conventional Commits, and how to use git tags to mark releases. It is consulted by `kata-tag` and `cry-tag`.

## Context

- **Domain:** Version identifiers, releases, and tags in Git
- **Audience:** AI agents running `kata-tag` and developers who publish releases
- **Update:** When the project's versioning convention changes
...
| Term | Definition |
|------|------------|
| MAJOR | First version number; incompatible changes |
| MINOR | Second version number; new compatible functionality |
| PATCH | Third version number; compatible fixes |
| Pre-release | Optional identifier after PATCH (e.g. alpha, beta, rc) |
| Build metadata | Optional metadata after `+`; does not affect precedence |
| Annotated tag | Git tag that stores an object with message and commit reference |
| Signed tag | Git tag signed with GPG for authenticity verification |

## References

- [Semantic Versioning 2.0.0](https://semver.org/)
- `lex-semantic-version` — Law requiring SemVer for releases
- `lex-signed-commits` — Law requiring GPG signature on release tags
- `codex-commit-standards` — Commit types and SemVer impact
- `kata-tag` — Procedure for creating compliant tags
- `cry-tag` — Recurring command to run git tag
