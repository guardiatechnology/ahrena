...
Create an isolated git worktree for a task, execute the work inside it, open the PR, and perform cleanup after merge — ensuring the main checkout remains clean and each task has its own dedicated environment.

## When to Use

- At the start of any task that requires a dedicated branch
- Before invoking warriors or katas that produce code or artifacts on branches
- When the user requests "implement X" and X requires a new branch
- When resuming an in-progress task that already has an existing worktree

## Inputs

| Input | Required | Description |
|-------|:--------:|-------------|
| Issue number | Yes | Existing GitHub Issue that originates the task (per `lex-issue-first`) |
| Branch type | Yes | One of: `feat`, `fix`, `docs`, `build`, `chore`, `ci`, `style`, `refactor`, `perf`, `test` |
| Slug | Yes | Short kebab-case description (max 50 chars) |
| Repository name | No | Default: root directory name of the repository |
...
```powershell
# 1. Exit the worktree (if currently inside)
Set-Location ../$repo
...
| Output | Description |
|--------|-------------|
| Worktree created | Directory `../{repo}-{issue-number}-{slug}/` with the active branch |
...
- `lex-agent-planning` — Task planning
