...
> Ensure that every delivered feature has tests at the right level, with real isolation and healthy distribution across the pyramid — because bugs in production always cost more than well-designed tests, and theatrical coverage is worse than honest coverage.
...
| `lex-test-pyramid` | 70/20/10 distribution per level |
| `lex-test-isolation` | Deterministic, parallelizable, flakiness-free tests |
| `lex-observability-required` | Test events also need observability where relevant |
| `lex-frontend-testing`, `lex-python-testing` | Stack-specific rules |

### Codex (Manuals it consults)

| Codex | Description |
|-------|-------------|
| `codex-test-strategy` | Decision tree by behavior type, anti-patterns, tools |
| `codex-python-testing` | pytest patterns, fixtures, Hypothesis |
| `codex-frontend-architecture` | Frontend layers (to decide where to test) |

### Katas (Procedures it executes)

| Kata | Description |
|------|-------------|
| `kata-test-plan-design` | Test plan design for a feature |
