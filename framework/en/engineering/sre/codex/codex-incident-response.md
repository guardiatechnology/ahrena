...
This Codex is the operational reference for **production incidents**. It defines severity levels, who to trigger at each level, how to structure communication (internal and external), when to rollback vs. forward-fix, and how to conduct blameless post-mortems. Consulted by `warrior-hestia` when orchestrating a response, by on-call engineers during the incident, and by leadership in communication with stakeholders.

## Context

- **Domain:** production incidents affecting critical services (tier-1/2).
- **Target audience:** `warrior-hestia`, on-call engineers, incident commanders, stakeholders
- **Update:** after each sev-1 incident (lessons learned); quarterly (procedure review)
...
- `lex-slo-required` — SLO and error budget guide post-incident prioritization
- `lex-runbook-for-every-alert` — runbook is step 1 of diagnosis
- `warrior-hestia` — leads response
- `kata-incident-triage`, `kata-postmortem-write` — detailed procedures
