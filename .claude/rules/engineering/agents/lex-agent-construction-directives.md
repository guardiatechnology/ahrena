...
- **Exceptions:** Lexis admit no exceptions. The only declared clause is the `legacy-pov` transition described below: agents created before this Lex's merge are treated as `stage: legacy-pov` and MUST migrate to `pre-operational` or `operational-concrete` within 90 days, with a retroactive DoOC plus an ADR recording the historical gap.
...
Single declared exception: agents created before this Lex's merge
are treated as `stage: legacy-pov`. Promotion to
`operational-concrete` requires a retroactive DoOC + ADR recording
the historical gap. The `legacy-pov` tag is NOT permanent: agents
in that stage MUST migrate to `pre-operational` or
`operational-concrete` within 90 days after this Lex's merge;
agents in `legacy-pov` beyond that deadline are considered
non-compliant.
</HARD-GATE>
```
