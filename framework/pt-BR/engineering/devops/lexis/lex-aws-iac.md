...
Mudanças manuais no console AWS são invisíveis, não versionadas, difíceis de auditar e impossíveis de reproduzir entre ambientes. Uma mudança "rápida" no console vira dívida técnica: ninguém sabe o estado real, ambientes divergem, rollback requer memória, e desastres viram acidentes sem rastro. Para agentes IA, aceitar mudanças manuais como padrão é construir fundação instável.

Esta Lexis existe para garantir que **toda infraestrutura AWS seja definida como código versionado**, que **mudanças passem por pull request revisado**, que **ambientes sejam reprodutíveis** e que **drift manual seja detectado e bloqueado**.
...
- **Aplica-se a:** toda infraestrutura AWS em todos os ambientes
- **Agentes vinculados:** `warrior-atlas`, agentes DevOps
- **Exceções:**
  - Recursos experimentais em conta sandbox podem ser manuais, se não persistem dados e são destruídos em 7 dias
  - Ações read-only no console para investigação (sem mudança de estado)

## Consequências de Violação

1. **Drift invisível:** ambiente real difere do código; deploys falham ou surpreendem
2. **Perda de conhecimento:** mudança manual some quando a pessoa sai; impossível reproduzir
3. **Falha de compliance:** SOC 2, ISO 27001 exigem trilha de mudança; console sem log é auditoria falha
4. **Remediação:**
   - Executar `terraform plan` para detectar drift; importar ou ajustar código
   - Aplicar SCPs restringindo mudanças manuais em produção
   - Auditoria de CloudTrail para detectar quem fez mudanças manuais recentes
...
- `lex-aws-security` — segurança como parte do IaC
- `lex-aws-cost` — tagging para cost allocation
- `codex-aws-well-architected` — Pilar de Excelência Operacional
