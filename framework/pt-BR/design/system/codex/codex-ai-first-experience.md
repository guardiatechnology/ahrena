...
A experiência da Guardia é **agêntica por padrão**. O Isac é o centro da interação — não um assistente acoplado a um SaaS clássico. O usuário descreve o resultado; o agente planeja, executa e presta contas. A UI existe para tornar isso legível, controlável e auditável. Este Codex consolida princípios, padrão de layout, regras de uso, exemplos e implicações para o Design System.

## Contexto

- **Domínio:** experiência do usuário em produto principal (plataforma e app), com presença permanente do Isac.
- **Público-alvo:** designers, frontend, mobile, produto, agentes de IA que produzam UI (warrior-hephaestus, warrior-iris).
- **Atualização:** quando a página *AI-First Experience* no Notion for revisada.
