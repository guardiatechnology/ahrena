...
> "Receive backend requests, identify the runtime first and then the `component`, delegate to the matching specialist, and coordinate multiple specialties when a change crosses boundaries."
...
| `lex-clean-code` | Objective hygiene shared by all stacks |
...
| `codex-dotnet-engineering` | Reference used by the .NET specialist |
...
| `warrior-apollo-dotnet` | Explicit .NET runtime or detection from project files/metadata; the specialist handles APIs, workers, and libraries within the stack |
