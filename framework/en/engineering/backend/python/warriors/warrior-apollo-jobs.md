...
> "Ensure that every Lambda handler or Step Function task in `components/jobs/` is idempotent, typed, instrumented, and testable — using AWS Lambda Powertools as the backbone, consuming CloudEvents correctly, and producing stable outputs that honor the Step Function's I/O schema."
...
| `lex-python-result-type` | Fallible functions return `Result[T, Error]`; raise only for permitted cases |
| `lex-python-error-object` | `Error` frozen dataclass with `code`/`reason`/`message`; no extra fields |
| `lex-python-error-handling` | No bare except; Lambda handler as boundary that logs + translates into `Error` |
| `lex-python-security` | No secrets in code (uses Secrets Manager / Parameter Store); dependency audit |
| `lex-python-testing` | Mocks only at the AWS boundary (`moto`); no mocking of internal collaborators |
| `lex-idempotency` | Required `idempotencykey` on consumed and published events; Powertools `@idempotent` |
| `lex-cloudevents` | CloudEvents 1.0 schema required; size < 12KB |
| `lex-observability-required` | Trace + metric + structured log with correlation ID on every handler |
| `lex-logging-decorator` | No inline `logger.info`; via Powertools Logger or centralized decorator |
| `lex-error-handling` | Emitted errors follow `ERR{HTTP}_` prefix even in asynchronous flows (`code` field in DLQ) |
| `lex-feature-design-docs` | `docs/{context}/events/events.md` is the canonical event contract source |

### Codex (Manuals consulted)

| Codex | Description |
|-------|-------------|
| `codex-component-jobs` | Internal layout of `components/jobs/`, canonical stack, BatchProcessor, idempotency store |
| `codex-component-architecture` | Boundaries between `api/`, `jobs/`, `agents/`, `ui/`, `deployment/` |
| `codex-python-architecture` | Clean Architecture applied to serverless |
| `codex-python-fastapi` | When the handler also serves a local ECS route, shared patterns |
| `codex-python-sqlalchemy` | Async 2.0 patterns when the job touches the DB |
| `codex-python-testing` | pytest, fixtures, `moto`, `pytest-asyncio` |
| `codex-python-observability` | Powertools Tracer/Logger/Metrics, `traceparent` propagation |
| `codex-python-tooling` | Ruff, mypy strict, uv, pre-commit |
| `codex-cloudevents` | Schema, idempotencykey, `event.guardia.{module}.{entity_type}.{event_name}` type format |
| `codex-aws-services` | EventBridge, SQS, Step Functions, DynamoDB idempotency store, workload-based choice |
| `codex-known-errors` | Platform `code`/`reason` catalog |
| `codex-feature-design-docs` | `events/events.md` category under `docs/{context}/` |

### Katas (Procedures executed)

| Kata | Description |
|------|-------------|
| `kata-python-implement` | End-to-end Python implementation (domain → handler → tests) |
| `kata-python-review` | Python review focused on idempotency, retry semantics, observability |
| `kata-python-refactor` | Safe refactoring with coverage as a safety net |
| `kata-python-debug` | Diagnosis (reproduce with `moto`, isolate, fix, regression test) |
