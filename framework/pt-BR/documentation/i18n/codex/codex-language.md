...
Este Codex fornece orientações práticas para traduzir documentação técnica em qualquer combinação de idiomas. É o guia transversal que complementa a `lex-language` (lei de tradução) e antecede os guias específicos por idioma (`codex-language-ptbr`, `codex-language-en`, `codex-language-es`).

## Contexto

- **Domínio:** Tradução de documentação técnica
- **Público-alvo:** `warrior-translator` (Hermes) e qualquer agente que execute traduções
- **Atualização:** sempre que novas convenções transversais forem estabelecidas
...
- `lex-language` — Lei transversal de tradução
- `lex-language-ptbr`, `lex-language-en`, `lex-language-es` — Leis por idioma-alvo
- `codex-language-ptbr`, `codex-language-en`, `codex-language-es` — Guias por idioma-alvo
- `kata-translate` — Procedimento coordenador
- `warrior-translator` — Agente de tradução (Hermes)
