...
Stitch together the `implement → validate → package` cycle of a skill so that the result in `{paths.skills_dist}/{slug}.skill/` satisfies `lex-skill-project-structure` and `lex-skill-package-structure` without any manual edit of `.build/` or `.dist/`.

> "The skill is born in Hephaestus and Apollo; I stitch, validate, and seal the package."
