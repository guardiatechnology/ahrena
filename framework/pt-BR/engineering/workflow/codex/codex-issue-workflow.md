...
Este Codex é a referência operacional do fluxo **Issue-Driven Development** do Ahrena. Define as 7 fases do processo, os 2 gates de qualidade, o formato dos artefatos intermediários, a convenção de rastreabilidade entre critérios de aceitação e testes, o formato dos ADRs (Architecture Decision Records) e a estrutura de documentação em `docs/`. Consultado por `warrior-athena` e por todos os katas do clade `engineering/workflow/`.

## Contexto

- **Domínio:** fluxo de entrega de features e bugfixes partindo de issues do GitHub, com orquestração via `warrior-athena`.
- **Público-alvo:** `warrior-athena`, katas do clade `engineering/workflow/`, e warriors especialistas delegados (Apollo, Daedalus, Kronos).
- **Atualização:** quando fases forem adicionadas/removidas, quando critérios do Gate 2 mudarem, ou quando a estrutura de `docs/` evoluir.
...
|:-:|---|---|---|
| 1 | Análise da issue | `kata-issue-analysis` | `docs/issues/issue-{n}/01-brief.md` |
| 2 | Elicitação de requisitos | `kata-requirements-brief` | `docs/issues/issue-{n}/02-requirements.md` |
| 3 | Design arquitetural | `kata-architecture-brief` (+ `kata-adr-write` se aplicável) | `docs/issues/issue-{n}/03-architecture.md` + `docs/adr/ADR-*` |
| 4 | Implementação | delega a `warrior-apollo` → `kata-python-implement` (Python) | código + testes com marcação `AC-N` |
| 5 | Revisão de segurança | `kata-security-review` | `docs/issues/issue-{n}/05-security-review.md` |
| 6 | Gate de qualidade | `kata-quality-gate` | `docs/issues/issue-{n}/06-quality-report.md` |
...
- 6 verificações (todas ✅ para passar):

| # | Verificação | Como |
|:-:|---|---|
| 1 | Rastreabilidade AC → teste (bidirecional) | Parse dos testes procurando `AC-N` + cross-check com ACs da Fase 2 |
| 2 | Scope creep check | `git diff` vs. componentes declarados na Fase 3 |
| 3 | Best practices (Lexis aplicáveis) | Checklist por Lexis (ver tabela abaixo) |
| 4 | Testes executados | `pytest` (ou equivalente) sem falhas |
...
- `warrior-athena` — orquestrador do fluxo
- `cry-implement-issue` — ponto de entrada
- `kata-issue-analysis`, `kata-requirements-brief`, `kata-architecture-brief`, `kata-adr-write`, `kata-security-review`, `kata-quality-gate`, `kata-pr-prepare` — katas do fluxo
- `kata-mcp-github-read`, `kata-mcp-notion-read` — leitura de contexto externo
- `lex-mcp`, `codex-mcp-github`, `codex-mcp-notion` — uso dos MCPs
