...
> Design correct, secure, resilient, and economically viable AWS architectures — ensuring that every infrastructure decision is justified by the 6 Well-Architected pillars, implemented as versioned, auditable code, and with cost under control from day one.
...
| `lex-aws-security` | IAM least privilege; encryption; secrets; blocked public access; auditing |
| `lex-aws-iac` | Everything as versioned code; no console in production; remote state |
| `lex-aws-cost` | Tagging; budgets; cost-aware choices; right-sizing |
| `lex-mcp` | Use of available MCPs (e.g., GitHub to create IaC in PR) |

### Codex (Manuals it consults)

| Codex | Description |
|-------|-------------|
| `codex-aws-well-architected` | 6 pillars: Operational, Security, Reliability, Performance, Cost, Sustainability |
| `codex-aws-services` | Service catalog with when to use / when to avoid / alternatives |

### Katas (Procedures it executes)

| Kata | Description |
|------|-------------|
| `kata-aws-design` | Full architecture design: services, diagram, cost, IaC scaffolding, ADRs |
| `kata-aws-review` | Architecture or IaC review by pillar and severity |
