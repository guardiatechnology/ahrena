...
Produzir os dois arquivos canônicos de identidade exigidos por `lex-agent-design-docs`: `overview.md` (governança, stage, entry mode, tier, owner, features servidas) e `system-prompt.md` (4 blocos obrigatórios per `lex-system-prompt`). Os dois arquivos são fonte autoritativa para todo o restante do design — orchestrator, specialists, tools, memory, feedback, context-pack consomem `overview.md` para escopo e `system-prompt.md` para identidade.

Cobre a **Diretriz 01 — Identidade Clara** de `lex-agent-construction-directives` em rigor de produção.

## Quando Usar

- Sempre como **primeiro kata** após `kata-dooc-validate` retornar `go`
- Quando `cry-agent-design` invocou `warrior-metis` para uma promoção `pre-operational` → `operational-concrete`
- Quando um agent em `operational-concrete` precisa de revisão de identidade (mudança de escopo aprovada, mudança de owner, expansão de features servidas)

## Inputs

| Input | Obrigatório | Descrição |
|-------|:-----------:|-----------|
| `context` | Sim | Bounded Context (kebab-case) |
| `agent` | Sim | Slug do agent (kebab-case) |
| `tier` | Sim | `tier-1` \| `tier-2` \| `tier-3` \| `tier-4` (vindo de `kata-dooc-validate`) |
| `entry_mode` | Sim | `with-pov` \| `direct-entry` \| `legacy-pov` (vindo de `kata-dooc-validate`) |
| `owner` | Sim | Nome + papel + canal de escalonamento (vindo de `kata-dooc-validate`) |
| `--from-pov <path>` | Não | Path do PoV de origem; preenche `serves_features` e ancora identidade |
| `serves_features` | Sim | Lista de features que o agent serve; cada feature DEVE existir em `docs/{context}/features/` |
| `PR ref` | Sim | `owner/repo#NNN` para audit trail per `lex-agent-design-docs` |
...
- `system-prompt.md` — identidade canônica do agent (Diretriz 01)
- `orchestrator.md` — loop de orquestração e estados entre specialists
- `memory.md` — 3 camadas de memória (Diretriz 02)
- `tools.md` — catálogo tripartido (Diretriz 03)
- `feedback.md` + `metrics.md` — loop de feedback + SLO (Diretriz 04 + tier-1/2 SLO)
- `context-pack.md` — few-shot + anti-padrões (Diretriz 06)
- `guardrails.md` + `authorization.md` + `escalation.md` — escopo restrito + controles (Diretriz 05)
- `docs/{context}/dooc/{agent}.md` — snapshot da DoOC
- `lex-agent-construction-directives`, `lex-agent-design-docs`, `lex-system-prompt`
```

### Passo 3: Redigir system-prompt.md (4 blocos per `lex-system-prompt`)

O arquivo `system-prompt.md` é a fonte autoritativa do prompt executado em runtime. Os 4 blocos obrigatórios (per `lex-system-prompt`) são:
...
> **Stage:** `operational-concrete`
> **Tier:** {tier}
> **Source of truth:** este arquivo. Quaisquer prompts inline em `orchestrator.md` ou `specialists/{name}.md` referenciam fragmentos deste documento, NÃO o contradizem.
> **Versionamento:** mudanças neste arquivo DEVEM passar por `kata-system-prompt-adversarial-validate` (suite completa) antes de merge.
