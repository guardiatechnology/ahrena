...
Duplicated domain knowledge diverges over time. One copy receives the fix, another does not, and the system enters a silent inconsistency that surfaces only in production (incorrect tax calculation, validation accepted in one flow and rejected in another, error mapping that differs across services). This Law eliminates that cost where it is real (rules that change together) without falling into premature abstraction (false-DRY), preserving the rule of three as an objective extraction trigger and requiring an explicit architectural decision (ADR) when the same knowledge crosses bounded contexts.
