...
> Ensure that every new HTTP API on the Guardia platform is designed consistently with RESTful Lexis and Codex, **in iterative dialogue with the user**, asking clarifying questions and refining the design until it meets the required criteria, producing an OpenAPI specification — clear, complete, and ready for implementation.
...
| `lex-feature-design-docs` | Canonical persistence at `docs/{context}/oas/openapi.yaml` |
| `lex-restful-apis` | RESTful compliance for HTTP endpoints |
| `lex-entities` | Base entity structure |
| `lex-idempotency` | Idempotency for state-modifying operations |
| `lex-error-handling` | Standardized error structure |
| `lex-auth` | Authentication and authorization for APIs |

### Codex (Manuals consulted)

| Codex | Description |
|-------|-------------|
| `codex-restful-apis` | RESTful APIs index and guidelines |
| `codex-restful-status-codes` | HTTP status codes and when to use |
| `codex-restful-payload` | data, pagination, errors, debug structure |
| `codex-restful-headers` | Standard and custom headers |
| `codex-restful-pagination` | Pagination parameters and response |
| `codex-restful-sorting` | order_by, sort |
| `codex-entities` | Entity model |
| `codex-idempotency` | Idempotency in APIs and events |
| `codex-error-handling` | Error handling |
| `codex-auth` | Authentication and authorization |
| `codex-oas-structure` | Order of operations in OAS paths (POST, GET, PUT, PATCH, DELETE) |
| `codex-feature-design-docs` | Template and structure for `docs/{context}/oas/openapi.yaml` |

### Katas (Procedures executed)

| Kata | Description |
|------|-------------|
| `kata-api-design-oas` | API design and OpenAPI 3.x specification production |
| `kata-feature-design-docs` | Persistence of the specification at the canonical path `docs/{context}/oas/openapi.yaml` |
