...
Executar uma verificação determinística do projeto de skill que rejeite, antes do commit, qualquer divergência em relação à `lex-skill-project-structure` e ao frontmatter mínimo do `SKILL.md`. O resultado é uma lista estruturada de violações (regra, severidade, arquivo, mensagem) consumível por humano (texto) ou por agente (JSON). Este kata **não** modifica arquivos — apenas reporta.

## Quando Usar

- Logo após `cry-new-skill` / `kata-init-skill`, antes da primeira autoria substantiva
- Ao concluir `kata-skill-implement`, antes de invocar `kata-skill-package`
- Em pre-commit hook do repositório que hospeda skills
- Quando `warrior-claudionor` precisa decidir se uma skill está pronta para empacotamento

## Inputs

| Input | Obrigatório | Descrição |
|-------|:-----------:|-----------|
| `skill_path` | Sim | Caminho do diretório raiz do projeto (`{paths.skills_root}/{slug}/`); idêntico ao nome do diretório (slug) |
| `format` | Não | `text` (default, humano) ou `json` (consumível por agente) |
...
- `scripts/skills/validate.py` — implementação determinística invocada
- `lex-skill-project-structure` — lei verificada
- `codex-skill-anthropic-agent-skills` — regras de frontmatter
- `lex-semantic-version` — formato de `metadata.version`
- `warrior-claudionor` — orquestrador que invoca este kata
- `cry-skill` — atalho `--mode validate`
- `kata-skill-package` — sucessor invocado após validate retornar `ok`
